...
class HalfWaveRectifier(gr.sync_block):
    ...
    def work(self, input_items, output_items):
        ...
        if not self._enabled:
            ...
            np.maximum(in0, 0.0, out)
        return len(in0)


class HilbertEnvelopeExtractor(gr.sync_block):
    """
    Replaces the audio with its slowly-varying amplitude envelope via a
    20ms IIR magnitude tracker.  Zero-crossing detection on this signal
    fires only at syllable onset and offset -- very few pulses per second.
    """
    def __init__(self, sample_rate=48000, tc=0.020):
    ...
    def work(self, input_items, output_items):
        ...
        if not self._enabled:
            out[:] = in0; return len(in0)
        alpha, env = self._alpha, self._env
        for i in range(len(in0)):
            env    = alpha * abs(float(in0[i])) + (1.0 - alpha) * env
            out[i] = env
        self._env = env
        return len(in0)
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/25 - Version 52)")
    ...
    def _build_gui(self):
        ...
        opt_layout.setColumnStretch(0, 1); opt_layout.setColumnStretch(1, 1)

        # -- Left column: existing filters (shortened labels) ----------------
        self._chk_notch = QtWidgets.QCheckBox("50/60 Hz Notch")
        self._chk_notch.setToolTip(
            "Narrow biquad notch filter (Q=30) centred on 50 or 60Hz mains\n"
            "hum and its harmonics.  Useful when the microphone picks up\n"
            "electrical interference from power wiring.")
        ...
        self._chk_preemph.setToolTip(
            ...
            "Sharpens consonants and sibilants, improving intelligibility of\n"
            "the pulse-encoded speech at the receiver.")
        ...
        # -- Right column: new pre-processing filters -------------------------
        self._chk_env_follow = QtWidgets.QCheckBox("Envelope Follower")
        self._chk_env_follow.setToolTip(
            "5ms IIR envelope gate: passes the signal when short-term\n"
            "amplitude exceeds a threshold, outputs zero during pauses and\n"
            "quiet regions.  Smoother than the instantaneous Noise Gate --\n"
            "preserves phoneme transitions while reducing noise-driven ZCR.")
        self._chk_env_follow.toggled.connect(self._toggle_env_follow)
        opt_layout.addWidget(self._chk_env_follow, 0, 1)

        self._chk_hwrect = QtWidgets.QCheckBox("Half-wave Rectification")
        self._chk_hwrect.setToolTip(
            "Clips all negative audio samples to zero.  The waveform then\n"
            "only transitions from positive down to zero (one direction),\n"
            "roughly halving the zero-crossing rate with no other signal\n"
            "changes.")
        ...
        self._chk_hilbert_env.setToolTip(
            "Replaces audio with its slowly-varying amplitude envelope via\n"
            "a 20ms IIR magnitude tracker (Hilbert approximation).\n"
            "Zero-crossing detection on this signal fires only at syllable\n"
            "onset and offset, dramatically reducing pulse rate to match\n"
            "speech rhythm rather than the carrier frequency.")
    ...
    def _cb_generate_espeak(self):
        ...
        if use_mbrola:
            ...
            # Formant synthesis: pitch 20 (default 50) reduces ZCR significantly
            cmd += ['-p', '20']
