...
# =============================================================================
#  Sliding Log Panel
#  Hangs off the right edge of the main window.
#  Tab has rounded right corners painted semi-transparent blue.
#  Slides open/closed with QPropertyAnimation.
# =============================================================================

class LogPanel(QtWidgets.QWidget):
    TAB_W   = 26    # width of the visible tab handle
    LOG_W   = 660   # width of the log content area (~90 mono chars)
    ANIM_MS = 280   # animation duration (ms)
    CORNER  = 10    # tab corner radius
    ...
    def __init__(self, parent_window):
        ...
        # -- Content widget (solid black, hidden when collapsed) -------------
        ...
        # Poll parent position every 50ms -- reliable on all Linux WMs
        # because moveEvent only fires at drag-end on most compositors.
        self._track_timer = QtCore.QTimer(self)
        self._track_timer.timeout.connect(self._track_parent)
        self._track_timer.start(50)

    # -- Geometry helpers --------------------------------------------------

    def _rect(self, expanded):
        # mapToGlobal gives the true screen position of the parent's right edge,
        # correctly accounting for window decorations and the window manager.
        pw       = self._pw
        top_right = pw.mapToGlobal(QtCore.QPoint(pw.width(), 0))
        x = top_right.x()
        y = top_right.y()
        h = pw.height()
        w = self.TAB_W + (self.LOG_W if expanded else 0)
        return QtCore.QRect(x, y, w, h)

    def _place(self):
        self.setGeometry(self._rect(self._expanded))
        if self._expanded:
            self._content.setGeometry(
                self.TAB_W, 0, self.LOG_W, self.height())

    def sync_with_parent(self):
        """Called from parent moveEvent / resizeEvent as a secondary trigger."""
        if not self._anim.state() == QtCore.QAbstractAnimation.Running:
            self._place()

    def _track_parent(self):
        """Poll-based tracking: moves the panel to follow the parent window."""
        if self._anim.state() == QtCore.QAbstractAnimation.Running:
            return   # don't interrupt an in-flight animation
        target = self._rect(self._expanded)
    ...
    def toggle(self):
        self._anim.stop()
        going_open = not self._expanded
        self._expanded = going_open
        if going_open:
            self._content.setGeometry(
                self.TAB_W, 0, self.LOG_W, self.height())
            self._content.show()
        self._anim.setStartValue(self.geometry())
        self._anim.setEndValue(self._rect(self._expanded))
        self._anim.start()
        self.update()
    ...
    # -- Painting: semi-transparent tab with rounded right corners ---------
    ...
    def paintEvent(self, event):
        ...
        # Tab shape: straight left side (flush with main window right edge),
        ...
        fill = QtGui.QColor(42, 110, 187, 195)
        p.setPen(QtCore.Qt.NoPen)
        p.setBrush(fill)
        p.drawPath(path)

        # Subtle highlight border on the right/visible edges
        pen = QtGui.QPen(QtGui.QColor(100, 160, 230, 160), 1)
        p.setPen(pen); p.setBrush(QtCore.Qt.NoBrush)
        p.drawPath(path)

        # "LOG" label rotated so it reads top to bottom
        ...
        label = "< LOG" if self._expanded else "LOG >"
        p.drawText(0, 0, int(h), int(w), QtCore.Qt.AlignCenter, label)

    # -- Input: click on tab area toggles ---------------------------------

    def mousePressEvent(self, event):
        if event.x() < self.TAB_W:
            self.toggle()
        else:
            super().mousePressEvent(event)

    def closeEvent(self, event):
        """Panel cannot be closed independently."""
        event.ignore()
    ...
    def log_error(self, context, exc):
        ...
        self.log("ERROR [{}]: {}".format(context, str(exc)[:120]))
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 30)")
        ...
        # Log panel -- created after GUI is shown so position is correct
        self._log_panel = None   # set in showEvent
    ...
    def showEvent(self, event):
        ...
        if self._log_panel is None:
            ...
            self._log_panel.log("Ready")

    def moveEvent(self, event):
        super().moveEvent(event)
        if self._log_panel:
            self._log_panel.sync_with_parent()
    ...
    def _log(self, msg, in_progress=False):
        if self._log_panel:
            self._log_panel.log(msg, in_progress)
    ...
    def _cb_tx_toggle(self, disabled):
        if disabled:
            ...
            self._log("Transmitting on {:.0f} MHz".format(
                self._freq_hz / 1e6))
    ...
    def _cb_generate_espeak(self):
        ...
        if r.returncode != 0:
            ...
            self._log("ERROR eSpeak: {}".format(err)); return
        ...
        try:
            ...
            self._log("ERROR WAV read: {}".format(e)); return
        ...
        try:
            ...
            self._log("ERROR GR reload: {}".format(e)); return
    ...
    def _generate_waterfall(self, iq_path):
        try:
            ...
            if len(data) < 1024:
                ...
                self._log_bg("Waterfall: recording too short"); return
            ...
            self._log_bg("Waterfall saved: {}".format(os.path.basename(png_path)))
        except Exception as e:
            self._set_wf_status("Error: {}".format(e))
            self._log_bg("ERROR waterfall: {}".format(e))

    def _log_bg(self, msg, in_progress=False):
