...
def detect_hackrf():
    try:
        ...
        # Check for explicit "no device" messages before anything else
        ...
        # Device is present -- parse firmware version
        lines    = output.splitlines()
        firmware = None

        # Strategy 1: (API: marker is unique to the firmware output line
        for l in lines:
            if '(API:' in l or '(api:' in l.lower():
                m = re.search(r'(\d+\.\d+[\.\d]*)', l)
                if m:
                    firmware = m.group(1); break

        # Strategy 2: skip tool/library version lines; grab 4-digit-year version
        if firmware is None:
            for l in lines:
                ll = l.lower().lstrip()
        ...
        # Strategy 3: any non-tool line mentioning 'firmware'
        if firmware is None:
            for l in lines:
                ll = l.lower().lstrip()
                if ll.startswith('hackrf_info') or ll.startswith('libhackrf'):
        ...
        if firmware is None:
            firmware = 'check hackrf_info manually'
...
# =============================================================================
#  Section Header (tab-style with blue gradient)
# =============================================================================
...
class SectionHeader(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        dark = QtGui.QColor("#2a6ebb")
        grad = QtGui.QLinearGradient(0, 0, 0, self.height())
        grad.setColorAt(0.0, dark); grad.setColorAt(1.0, bg)
...
class SwapButton(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.setPen(pen)
        cx = self.width() // 2; mid = self.height() // 2
        half_d = self.BTN_D // 2
...
# =============================================================================
#  Audio level meter  (readout at top-right of bar)
# =============================================================================
...
class LevelMeter(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        # Top row: bar + dB readout right-aligned at far right
        ...
        self._readout = QtWidgets.QLabel("-60.0 dB")
        self._readout.setFont(QtGui.QFont("Monospace", 7))
        self._readout.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        self._readout.setFixedWidth(58)  # "B" in "dB" aligns with right edge
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    ESPEAK_SILENCE_SEC = 3.0   # silence padding after speech to normalise duty
    _DISABLED_BG  = QtGui.QColor("#b8b8b8")
    BTN_H         = 58         # standard button height (approx 2x default)
    ...
    # Regex: letters and spaces only, for eSpeak input
    _ESPEAK_RE = re.compile(r'^[a-zA-Z ]+$')

    def __init__(self):
        gr.top_block.__init__(self, "OpenV2K", catch_exceptions=True)
        QtWidgets.QMainWindow.__init__(self)
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 27)")
    ...
    def _push_status(self, msg):
        """Push a new status message; previous messages scroll down."""
        self._s3.setText(self._s2.text())
        self._s2.setText(self._s1.text())
        self._s1.setText(msg)
    ...
    def _build_gui(self):
        ...
        # -- Title row: status (left) | title (centre) | balance (right) ------
        top_row = QtWidgets.QHBoxLayout(); top_row.setSpacing(0)

        # Status area: 3 stacked labels, top-left corner
        status_w = QtWidgets.QWidget(); status_w.setFixedWidth(155)
        sv = QtWidgets.QVBoxLayout(status_w)
        sv.setContentsMargins(0, 0, 0, 0); sv.setSpacing(0)
        self._s1 = QtWidgets.QLabel("Ready")
        self._s1.setFont(QtGui.QFont("Monospace", 8))
        self._s1.setStyleSheet("color: black;")
        self._s2 = QtWidgets.QLabel("")
        self._s2.setFont(QtGui.QFont("Monospace", 8))
        self._s2.setStyleSheet("color: #555;")
        self._s3 = QtWidgets.QLabel("")
        self._s3.setFont(QtGui.QFont("Monospace", 8))
        self._s3.setStyleSheet("color: #aaa;")
        sv.addWidget(self._s1); sv.addWidget(self._s2); sv.addWidget(self._s3)
        top_row.addWidget(status_w)
        ...
        # Balance spacer (same width as status area)
        bal = QtWidgets.QWidget(); bal.setFixedWidth(155)
        top_row.addWidget(bal)
        ...
        mic_sub.setStyleSheet("font-size:9pt; font-weight:bold; color:black;")
        ...
        # Generate Voice button -- starts disabled (no text typed yet)
        ...
        self._dc_vbar.setRange(0, 1000)
        self._dc_vbar.setValue(0); self._dc_vbar.setTextVisible(False)
        self._dc_vbar.setFixedWidth(20)
        ...
        save_vbox.setContentsMargins(4, 4, 0, 4); save_vbox.setSpacing(4)
    ...
    def _build_blocks(self):
        ...
        self.resampler= gr_filter.rational_resampler_fff(
            ...
            decimation=self.RESAMP_DECIM, taps=[], fractional_bw=0.0)
    ...
    def _cb_audio_swap(self):
        ...
        if self._audio_left_active:
            ...
            self._push_status("Input: Live Microphone (muted)")
        else:
            self._set_panel_active(self._mic_panel, False)
            self._set_panel_active(self._es_panel,  True)
            self.mic_gate.set_k(0.0); self.espeak_gate.set_k(1.0)
            self._level_meter.freeze()
            # Re-evaluate generate button state after panel is re-enabled
            t = self._espeak_input.text().strip()
            self._btn_generate.setEnabled(
                len(t) > 0 and t.lower() != 'hello world')
            self._push_status("Input: eSpeak TTS")
    ...
    def _cb_output_swap(self):
        ...
        if self._output_left_active:
            ...
            self._btn_tx.setStyleSheet(self._style_red())
            self._push_status("Output: HackRF Transmitter (disabled)")
        else:
            self.tx_gate.set_k((0+0j))
            if not self._btn_tx.isChecked(): self._btn_tx.setChecked(True)
            self._set_panel_active(self._tx_panel,   False)
            self._set_panel_active(self._save_panel, True)
            self._push_status("Output: Save to Disk")
    ...
    def _cb_mute(self, muted):
        ...
        self._push_status(
            "Microphone muted" if muted else "Microphone live -- monitoring")
    ...
    def _cb_tx_toggle(self, disabled):
        if disabled:
            ...
            self._btn_tx.setText("TX: DISABLED")
            self._btn_tx.setStyleSheet(self._style_red())
            self._push_status("Transmitter disabled")
        else:
            ...
            self._push_status(
                "Transmitting on {:.0f} MHz".format(freq_mhz))

    # =========================================================================
    #  eSpeak TTS
    # =========================================================================

    def _on_espeak_text_changed(self, text):
        """Enable Generate Voice only when real (non-placeholder) text is typed."""
        stripped = text.strip()
        enabled  = (len(stripped) > 0
                    and stripped.lower() != 'hello world'
                    and bool(self._ESPEAK_RE.match(stripped)))
        self._btn_generate.setEnabled(enabled)

    def _cb_generate_espeak(self):
        text = self._espeak_input.text().strip()
        if not text or not self._ESPEAK_RE.match(text):
            return

        self._push_status("Generating eSpeak audio...")
        ...
        try:
            ...
            self._push_status("WAV read error"); return
        ...
        # Append silence to reduce average duty cycle during looping.
        # Without padding, synthetic speech loops at 10-20% duty cycle.
        # 3 seconds of silence brings the average into the 2-4% range.
        silence = np.zeros(
            int(self.AUDIO_RATE * self.ESPEAK_SILENCE_SEC),
            dtype=np.float32)
        ...
        self._push_status('eSpeak playing: "{}"'.format(short))
    ...
    def _cb_record_toggle(self, recording):
        if recording:
            ...
            self._push_status("Recording IQ to disk...")
        else:
            self.lock()
            self.disconnect(self.f2c, self.iq_recorder)
            self.iq_recorder = blocks.null_sink(gr.sizeof_gr_complex)
            ...
            self._push_status("Recording stopped")
