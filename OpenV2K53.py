...
def check_prerequisites():
    ...
    # MBROLA (optional)
    if _MBROLA_OK:
        print("{}mbrola + en1  (MBROLA synthesis)".format(OK))
    else:
        print("{}mbrola  ->  sudo apt install mbrola mbrola-en1".format(WARN))
...
class EnvelopeFollower(gr.sync_block):
    ...
    """
    def __init__(self, sample_rate=48000, tc=0.005, thresh=0.02):
...
class HalfWaveRectifier(gr.sync_block):
    ...
    def work(self, input_items, output_items):
        ...
        if not self._enabled:
            ...
            # Replace negatives with a tiny negative constant so ZCP still
            # detects sign changes at the start and end of each positive lobe.
            # Crossings WITHIN the negative half are eliminated (all -1e-6).
            out[:] = np.where(in0 > 0.0, in0, np.float32(-1e-6))
        return len(in0)
...
class SchmittFilter(gr.sync_block):
    ...
    """
    def __init__(self, hi=0.05, lo=-0.05):
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    ESPEAK_SILENCE_SEC = 1.0    # 1s silence on each end of the voice sample
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/25 - Version 53)")
        ...
        self._chk_hilbert_env.setChecked(True)
    ...
    def _build_gui(self):
        ...
        es_title = QtWidgets.QLabel("eSpeak TTS")
        ...
        self._chk_mbrola = QtWidgets.QCheckBox("MBROLA")
        self._chk_mbrola.setStyleSheet("color:black; font-size:9px;")
        self._keep_black(self._chk_mbrola)
        _mbrola_status = "Available" if _MBROLA_OK else \
            "Not installed -- sudo apt install mbrola mbrola-en1"
        ...
        self._sl_pulse = LabelledSlider("Pulse (\u00b5s)", 25, 150, 5, self._pulse_us,
            fmt="{:.0f} \u00b5s", callback=self._cb_pulse, tick_steps=10)
        left_vbox.addWidget(self._sl_pulse)
        self._sl_hpf = LabelledSlider("HPF (Hz)", 100, 300, 10, self._hpf_hz,
            fmt="{:.0f} Hz", callback=self._cb_hpf, tick_steps=5)
        left_vbox.addWidget(self._sl_hpf)
        self._sl_lpf = LabelledSlider("LPF (Hz)", 100, 3000, 100, self._lpf_hz,
        ...
        # -- Column 0: original filters (shortened labels) --------------------
        self._chk_notch = QtWidgets.QCheckBox("50/60 Hz Notch")
        self._chk_notch.setToolTip(
            "Narrow biquad notch filter (Q=30) centred on 50 or 60Hz mains hum\n"
            "and its harmonics.  Useful when the microphone picks up electrical\n"
            "interference from power wiring.")
        self._chk_notch.toggled.connect(self._toggle_notch)
        opt_layout.addWidget(self._chk_notch, 0, 0)

        self._chk_preemph = QtWidgets.QCheckBox("Pre-emphasis")
        self._chk_preemph.setToolTip(
            "+6 dB/octave high-frequency boost above 1kHz via FIR [1, -0.9375].\n"
            "Sharpens consonants and sibilants, improving intelligibility of the\n"
            "pulse-encoded speech.")
        self._chk_preemph.toggled.connect(self._toggle_preemph)
        opt_layout.addWidget(self._chk_preemph, 1, 0)

        self._chk_noisegate = QtWidgets.QCheckBox("Noise Gate")
        self._chk_noisegate.setToolTip(
            "Zeros the audio when instantaneous RMS power falls below -30dB.\n"
            "Prevents background noise and pauses from generating spurious\n"
            "zero-crossings at the ZCP block.")
        self._chk_noisegate.toggled.connect(self._toggle_noisegate)
        opt_layout.addWidget(self._chk_noisegate, 2, 0)

        # -- Column 1: v52 pre-processing filters -----------------------------
        self._chk_env_follow = QtWidgets.QCheckBox("Envelope Follower")
        self._chk_env_follow.setToolTip(
            "5ms IIR envelope gate: passes the signal when short-term amplitude\n"
            "exceeds threshold, outputs zero during pauses and quiet regions.\n"
            "Smoother than the instantaneous Noise Gate -- preserves transitions\n"
            "while reducing noise-driven ZCR.")
        self._chk_env_follow.toggled.connect(self._toggle_env_follow)
        opt_layout.addWidget(self._chk_env_follow, 0, 1)

        self._chk_hwrect = QtWidgets.QCheckBox("Half-wave Rect.")
        self._chk_hwrect.setToolTip(
            "Replaces negative samples with a tiny negative constant (-1e-6).\n"
            "Crossings WITHIN the negative half-cycle are collapsed to zero,\n"
            "eliminating harmonic ZCR contributions from negative lobes.\n"
            "Roughly halves ZCR for voiced speech.")
        self._chk_hwrect.toggled.connect(self._toggle_hwrect)
        opt_layout.addWidget(self._chk_hwrect, 1, 1)

        self._chk_hilbert_env = QtWidgets.QCheckBox("Hilbert Envelope")
        self._chk_hilbert_env.setToolTip(
            "Replaces audio with (envelope - slow_mean): positive during active\n"
            "speech, negative during pauses.  ZCP fires at syllable onset/offset\n"
            "rather than at carrier frequency -- dramatically fewer pulses that\n"
            "match the speech rhythm instead of the waveform crossings.")
        self._chk_hilbert_env.toggled.connect(self._toggle_hilbert_env)
        opt_layout.addWidget(self._chk_hilbert_env, 2, 1)

        # -- Column 2: new filters --------------------------------------------
        self._chk_decimator = QtWidgets.QCheckBox("Downsample / Decimate")
        self._chk_decimator.setToolTip(
            "Applies a 4kHz IIR anti-alias lowpass before ZCP.\n"
            "Equivalent to soft decimation from 48kHz to 8kHz effective rate.\n"
            "Removes high-frequency harmonic content above speech intelligibility\n"
            "range that generates the most spurious zero crossings.")
        self._chk_decimator.toggled.connect(self._toggle_decimator)
        opt_layout.addWidget(self._chk_decimator, 0, 2)

        self._chk_schmitt = QtWidgets.QCheckBox("Schmitt Trigger")
        self._chk_schmitt.setToolTip(
            "Applies hysteresis: snaps output to +0.5 when signal crosses above\n"
            "+0.05, and to -0.5 when it crosses below -0.05.  Eliminates rapid\n"
            "noise-driven toggling near zero.  ZCP only fires on decisive\n"
            "threshold crossings, not on noise jitter.")
        self._chk_schmitt.toggled.connect(self._toggle_schmitt)
        opt_layout.addWidget(self._chk_schmitt, 1, 2)

        self._chk_spectral_sub = QtWidgets.QCheckBox("Spectral Subtraction")
        self._chk_spectral_sub.setToolTip(
            "Estimates background noise floor during quiet periods and applies\n"
            "a Wiener-filter gain: gain near 0 when signal is at noise level,\n"
            "approaching 1 when signal is well above noise.  Reduces ZCR\n"
            "contribution from room noise and microphone hiss.")
        self._chk_spectral_sub.toggled.connect(self._toggle_spectral_sub)
        opt_layout.addWidget(self._chk_spectral_sub, 2, 2)
        ...
        pwr_lbl = QtWidgets.QLabel("TX Power:")
    ...
    def _connect_blocks(self):
        ...
        self.connect(self.env_follower, self.hwrect)
        self.connect(self.hwrect,       self.hilbert_env)
        self.connect(self.hilbert_env,  self.decimator)
        self.connect(self.decimator,    self.schmitt)
        self.connect(self.schmitt,      self.spectral_sub)
        self.connect(self.spectral_sub, self.zcp)
    ...
    def _toggle_env_follow(self, e):    self.env_follower.set_enabled(e)
    def _toggle_hwrect(self, e):        self.hwrect.set_enabled(e)
    def _toggle_hilbert_env(self, e):   self.hilbert_env.set_enabled(e)
    def _toggle_decimator(self, e):     self.decimator.set_enabled(e)
    def _toggle_schmitt(self, e):       self.schmitt.set_enabled(e)
    def _toggle_spectral_sub(self, e):  self.spectral_sub.set_enabled(e)
    ...
    def _cb_generate_espeak(self):
        ...
        if not text:
            self._espeak_input.setPlaceholderText("Enter Some Text Here")
            return
        ...
        use_mbrola = _MBROLA_OK and self._chk_mbrola.isChecked()
        cmd = [cmd_found]
        if use_mbrola:
            # MBROLA: -p (pitch) disrupts diphone synthesis -- omit it
            # -a 200 boosts amplitude (MBROLA voices are quieter than formant)
            cmd += ['-v', 'mb-en1', '-a', '200', '-s', '130']
        else:
            # Formant synthesis: pitch 20 reduces ZCR; speed 130 < default 175
            cmd += ['-p', '20', '-s', '130']
    ...
    def _generate_waterfall(self, iq_path):
        try:
            ...
            sr       = self.HACKRF_RATE
            fft_size = 64    # 32 us window -- resolves 100 us pulses when hop is small

            # ---- Locate the FULL active region (first to last active sample) -
            mag    = np.abs(data)
            thresh = max(mag.max() * 0.05, 1e-9)
            active = np.where(mag > thresh)[0]
            if len(active) == 0:
                self._log("Waterfall: no signal found in recording"); return

            pre   = int(sr * 0.001)   # 1 ms pre-roll before first pulse
            post  = int(sr * 0.001)   # 1 ms post-roll after last pulse
            ...
            # ---- Adapt hop to fit ~4000 rows regardless of phrase length ----
            # Short captures (<100 ms): hop stays at 32 (16 us -- pulses visible)
            # Long captures (1-3 s):    hop scales up; envelope stays clear
            target_rows = 4000
            hop    = max(32, (len(chunk) - fft_size) // target_rows)
            step_us = hop / sr * 1e6

            win     = np.hanning(fft_size).astype(np.float32)
            n_fr    = (len(chunk) - fft_size) // hop
            spec    = np.zeros((n_fr, fft_size), dtype=np.float32)
            for i in range(n_fr):
                fr       = chunk[i*hop : i*hop+fft_size]
                spec[i]  = 10.0 * np.log10(
                    np.fft.fftshift(np.abs(np.fft.fft(fr * win))**2) + 1e-10)

            t0_ms   = start / sr * 1000.0
            t1_ms   = (start + len(chunk)) / sr * 1000.0
            win_us  = fft_size / sr * 1e6

            # ---- Amplitude envelope (time domain) ---------------------------
            t_env = (np.arange(len(chunk)) + start) / sr * 1000.0  # ms
            ...
            fig.suptitle(
                ...
                'Active region {:.2f} ms - {:.2f} ms  |  '
                '{:.0f} us FFT window  /  {:.0f} us time step  '
                '(~{:.0f} rows per 100 us pulse)'
            ...
            # Spectrogram: rows = time (top=t0, bottom=t1), cols = frequency
            ...
            # Amplitude envelope -- shows pulse train directly
            ax2.plot(t_env, mag[start:end], color='#2a6ebb', linewidth=0.4)
