...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/25 - Version 49)")
    ...
    def _generate_waterfall(self, iq_path):
        try:
            ...
            sr       = self.HACKRF_RATE
            fft_size = 64    # 32 us window at 2 MHz -- fits inside a 100 us pulse
            hop      = 32    # 16 us time step -- 6 rows per 100 us pulse

            # ---- Locate first active region (skip lead silence) -------------
            mag    = np.abs(data)
            ...
            # 1 ms pre-roll before the first pulse, then 10 ms window
            pre    = int(sr * 0.001)
            win_n  = int(sr * 0.010)          # 10 ms = 20,000 samples @ 2 MHz
            start  = max(0, active[0] - pre)
            end    = min(len(data), start + win_n)
