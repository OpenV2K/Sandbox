...
def _check_mbrola():
    """Return True if the MBROLA en1 voice data is installed."""
    candidates = [
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 45)")
    ...
    def _cb_generate_espeak(self):
        ...
        # Build espeak command: pitch 20 (default 50) reduces ZCR significantly
        use_mbrola = _MBROLA_OK and self._chk_mbrola.isChecked()
        cmd = [cmd_found]
        if use_mbrola:
            cmd += ['-v', 'mb-en1']
        cmd += ['-p', '20', '-w', self.ESPEAK_RAW, text]
