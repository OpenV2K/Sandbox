...
class PulseHeaderWidget(QtWidgets.QWidget):
    ...
    MIN_SILENCE = 1
    MAX_SILENCE = 4          # was 6 -- narrower range, part of reducing
                            # the average gap between pulses
    HALF_TICK_PROB = 0.10    # ~10% of gaps are a half tick
    TICK_AND_HALF_PROB = 0.10   # ~10% of gaps are a tick and a half
                                # (remaining ~80% use the normal
                                # MIN_SILENCE..MAX_SILENCE random range)

    # Periodic density burst: every ~1500ms, pulses run denser for
    # ~300ms, then back to normal.  Same random.randint(min, max) call
    # as always -- during a burst, just MAX_SILENCE shrinks to
    # BURST_MAX_SILENCE (min stays the same), so the same dice roll
    # naturally lands on short gaps more often.  The bitstream is
    # generated once as a spatial pattern (not live), so "every 1500ms"
    # is converted to a pixel distance using the estimated scroll speed
    # (average segment width / SCROLL_PERIOD_S) -- an approximation,
    # not an exact real-time guarantee, given the pattern loops and its
    # exact final length varies slightly run to run.
    BURST_MAX_SILENCE = 2   # was going to be 1, but that collapses
                            # randint(1,1) to a single deterministic
                            # value -- 2 keeps genuine randomness (still
                            # picks between 1 or 2 ticks) while staying
                            # clearly denser than the normal 1-4 range
    BURST_PERIOD_PX = 105.0    # ~1500ms -- recalibrated empirically
                              # (the original theoretical estimate of
                              # 145px, based on an average-segment-width
                              # calculation, measured ~38% slow when
                              # actually simulated; this value was
                              # verified by generating real sequences
                              # and measuring actual burst-to-burst
                              # spacing against the target)
    BURST_DURATION_PX = 21.0   # ~300ms -- same empirical recalibration
    ...
    def _generate_bitstream(self):
        ...
        Each silence gap is usually a random 1-MAX_SILENCE ticks
        (~80% of the time), but ~10% of the time is a half tick and
        ~10% of the time is a tick and a half -- occasional fractional
        gaps alongside the normal whole-tick ones.

        Periodic density burst: every ~1500ms-equivalent of generated
        distance, gaps run denser for ~300ms-equivalent, then back to
        normal.  Same randint(min, max) call throughout -- during a
        burst, MAX_SILENCE is simply swapped for the smaller
        BURST_MAX_SILENCE (MIN_SILENCE stays the same), so the same
        dice roll naturally lands on short gaps more often, with no
        separate weighting scheme needed.  The fractional (half-tick /
        tick-and-a-half) logic is skipped during a burst -- just the
        plain randint call with the tightened range.
        ...
        while total_w < target_w:
            ...
            if in_burst:
                gap_ticks = float(rng.randint(self.MIN_SILENCE,
                                              self.BURST_MAX_SILENCE))
            else:
                roll = rng.random()
                if roll < self.HALF_TICK_PROB:
                    gap_ticks = 0.5
                elif roll < self.HALF_TICK_PROB + self.TICK_AND_HALF_PROB:
                    gap_ticks = 1.5
                else:
                    gap_ticks = float(rng.randint(self.MIN_SILENCE, self.MAX_SILENCE))
            gap_px = gap_ticks * self.TICK_PX
            segments.append((False, gap_px))
            total_w += gap_px

            dist_in_phase += self.PULSE_WIDTH_PX + gap_px
            if not in_burst and dist_in_phase >= self.BURST_PERIOD_PX:
                in_burst = True
                dist_in_phase = 0.0
            elif in_burst and dist_in_phase >= self.BURST_DURATION_PX:
                in_burst = False
                dist_in_phase = 0.0
        return segments
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 148)")
    ...
    def _build_gui(self):
        ...
        # White drop shadow, offset slightly down-right, minimal blur --
        # gives the text an embossed/pressed-in look against the moving
        # pulse background behind it (a dark shadow would read as raised
        # instead; a light one offset this way reads as recessed).
        _title_shadow = QtWidgets.QGraphicsDropShadowEffect()
        _title_shadow.setColor(QtGui.QColor(255, 255, 255, 235))
        _title_shadow.setOffset(1, 1)
        _title_shadow.setBlurRadius(1)   # minimal -- fairly hard edge,
                                         # not a soft glow
