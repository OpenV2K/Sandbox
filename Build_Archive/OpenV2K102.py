...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/12 - Version 102)")
    ...
    def _update_power_calc(self):
        ...
        # above -- power delivered with no free-space spreading loss.
        # 50cm/1m apply the standard isotropic-radiator power-density
        # formula, S(r) = P / (4*pi*r^2), rearranged to solve for the
        # transmitter power needed to reproduce the SAME power density at
        # distance r: P(r) = P_point_blank * 4*pi*r^2.  This assumes an
        # idealized isotropic (omnidirectional) radiator in free space --
        # no antenna gain, no atmospheric loss -- so treat these as
        # illustrative upper/lower bounds, not engineering figures.
        # 50cm/1m chosen over 1m/2m as more realistic reference distances
        # given the achievable power levels at typical pulse widths.
        p_50cm = p_point_blank * 4.0 * math.pi * (0.5 ** 2)
        p_1m   = p_point_blank * 4.0 * math.pi * (1.0 ** 2)
        self._power_recommended_lbl.setText(
            "{} {:.0f}W (Point Blank), {:.0f}W (50cm), {:.0f}W (1m)".format(
