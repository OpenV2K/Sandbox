...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/12 - Version 94)")
    ...
    def _retranslate_ui(self):
        ...
        self._power_calc_title_lbl.setText(_tr("power_calculation"))
    ...
    def _build_gui(self):
        ...
        # Signal Conditioning grew to 6 rows).
        #
        # Title + Reset link live OUTSIDE the bordered box (in their own
        # row above it), unlike Optional Filters' native embedded title --
        # this lets Reset sit directly beside the title text, which a
        # native QGroupBox title (style-drawn, not a widget) can't support.
        pc_container = QtWidgets.QWidget()
        pc_container.setSizePolicy(
            QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        pc_outer = QtWidgets.QVBoxLayout(pc_container)
        pc_outer.setContentsMargins(0, 8, 0, 0); pc_outer.setSpacing(2)

        pc_title_row = QtWidgets.QHBoxLayout()
        pc_title_row.setContentsMargins(0, 0, 0, 0)
        self._power_calc_title_lbl = QtWidgets.QLabel(_tr("power_calculation"))
        self._power_calc_title_lbl.setStyleSheet(
            "font-size:9pt; color:#8B0000;")   # dark red, not bold
        pc_title_row.addWidget(self._power_calc_title_lbl)
        pc_title_row.addSpacing(14)   # ~2-3 spaces -- Reset sits right beside it
        self._power_reset_lbl = QtWidgets.QLabel(
            "<a href='#' style='color:#2a6ebb; text-decoration:none;'>{}</a>"
            .format(_tr("power_reset")))
        self._power_reset_lbl.setStyleSheet("font-size:8pt;")
        self._power_reset_lbl.linkActivated.connect(self._cb_power_reset)
        pc_title_row.addWidget(self._power_reset_lbl)
        pc_title_row.addStretch(1)   # remaining row width stays empty at the end
        pc_outer.addLayout(pc_title_row)

        # Plain bordered box, no native title -- title now lives above it.
        # Expanding size policy so it stretches to fill its full grid cell --
        # bottom and right edges sit flush with the grid's own boundary.
        self._power_calc_box = QtWidgets.QGroupBox()
        self._power_calc_box.setSizePolicy(
            QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        self._power_calc_box.setStyleSheet(
            "QGroupBox { font-size:9pt; margin-top:2px; }")
        pc_vbox = QtWidgets.QVBoxLayout(self._power_calc_box)
        pc_vbox.setContentsMargins(6, 6, 6, 6); pc_vbox.setSpacing(4)

        # Row 1: Session Pulse Count + Total Energy Output (3 power tiers)
        self._power_count_lbl = QtWidgets.QLabel()
        self._power_count_lbl.setStyleSheet("font-size:9px; color:#333;")
        ...
        self._power_perpulse_lbl.setStyleSheet("font-size:9px; color:#333;")
        self._power_perpulse_lbl.setWordWrap(True)
        pc_vbox.addWidget(self._power_perpulse_lbl)

        pc_vbox.addStretch(1)
        pc_outer.addWidget(self._power_calc_box)
        opt_layout.addWidget(pc_container, 4, 1, 3, 2)
        ...
        self._save_path_lbl.setMinimumHeight(115)
    ...
    def _update_power_calc(self):
        ...
        Joule values use dense fixed-decimal formatting: ##.##J (@2kW),
        which is compact enough to avoid word-wrap in longer languages
        while staying easier to scan at a glance than scientific notation.
        """
        ...
        # Row 1: Session Pulse Count + Total Energy Output (2 power tiers,
        # each tier's total = count x energy-per-pulse-at-that-tier)
        ...
        self._power_count_lbl.setText(
            "{} {}   {} {}".format(
                ...
                _tr("power_total_energy"), ", ".join(total_parts)))

        # Row 2: Power Per Pulse (2 power tiers)
        perpulse_parts = [_fmt_j(pulse_s * p_w, p_w)
                          for p_w in self._POWER_LEVELS_W]
        self._power_perpulse_lbl.setText(
            "{} {}".format(_tr("power_per_pulse"), ", ".join(perpulse_parts)))
