...
class WaveHeaderWidget(QtWidgets.QWidget):
    """
    Animated 3-layer parallax 'waves divider' background, recreating a
    popular CSS/SVG effect: one repeating wave shape, drawn multiple
    times at different scroll speeds/directions, layered to create a
    drifting ocean look.  Paints in its OWN paintEvent -- which Qt
    renders BEFORE child widgets -- so any child widget placed inside it
    (e.g. the OpenV2K title label) sits on top, unobstructed.  Same
    "paint behind, let children render on top" technique already used
    for SwapButton's domed highlight.

    Wave geometry: one QPainterPath built from the original SVG path's
    own cubic-bezier control points (M-160 44 c... s... s... s...),
    decoded to two curves spanning one full peak-trough-peak cycle
    (176 SVG units wide), tiled horizontally to cover the widget and
    scrolled via a QTimer.  Transparent background -- an earlier version
    filled it with an explicit white rect for guaranteed contrast, but
    that created a visible white box seam against the surrounding UI
    chrome's real background, so contrast now comes entirely from the
    wave colours themselves being dark enough to read against most
    light-theme backgrounds without needing a background fill at all.

    Horizontal scale (wavelength) and vertical scale (crest height) are
    independent, and the trough sits at an EXPLICIT pixel position
    (TROUGH_Y_PX) rather than being derived from a single uniform scale
    -- so crest height and wavelength can each be pushed much further
    without disturbing the trough's own position near the bottom edge.

    Layers 2 and 3 (of 3) get an added phase offset and a slow secondary
    sine-wave "wobble" layered on top of their base linear scroll, so
    their motion reads as more varied/organic rather than perfectly
    mechanical -- layer 1 (the backmost, slowest layer) stays a plain
    linear scroll.
    """
    # SVG units: y=26 is peak, y=44 is trough (18-unit amplitude), one
    # period is 176 units wide.  Curve control points decoded directly
    # from the original "M-160 44c30 0 58-18 88-18s58 18 88 18..." data.
    PEAK_Y_SVG = 26.0
    TROUGH_Y_SVG = 44.0
    AMPLITUDE_SVG = TROUGH_Y_SVG - PEAK_Y_SVG   # 18.0
    PERIOD_UNITS = 176.0

    SCALE_X = 1.8      # was 0.75 -- much wider wavelength (317px period)
    AMPLITUDE_PX = 50.0   # was 11.7px -- much higher crests
    TROUGH_Y_PX = 66.0    # explicit position: still 4px above the bottom
                          # edge (of the now-taller 70px widget) --
                          # trough depth itself is unchanged by the
                          # amplitude/wavelength increases above

    # (period_seconds, direction, grey_color, phase_offset_frac,
    #  wobble_period_s, wobble_amplitude_frac) -- back to front.  Only 3
    # layers now (was 4) -- spread across a wider greyscale range so
    # each is clearly distinguishable with one fewer layer to share the
    # spectrum.  Layers 2 and 3 (index 1, 2) carry a phase offset and a
    # secondary wobble frequency layered onto their base scroll, for a
    # more varied, less mechanically-synchronized look; layer 1 (index
    # 0, the slowest/backmost) stays a plain linear scroll.
    _LAYERS = [
        (10.0, +1, QtGui.QColor(215, 215, 215), 0.00, 0.0,  0.00),
        ( 8.0, -1, QtGui.QColor(175, 175, 175), 0.35, 3.7,  0.18),
        ( 6.0, +1, QtGui.QColor(140, 140, 140), 0.62, 5.3,  0.24),
    ]
    ...
    def _y(self, y_svg):
        """Map an SVG-space y-coordinate (26=peak, 44=trough) to a pixel
        y-coordinate, using the independent vertical scale and the
        explicit trough position rather than a single uniform scale."""
        scale_y = self.AMPLITUDE_PX / self.AMPLITUDE_SVG
        return self.TROUGH_Y_PX - (self.TROUGH_Y_SVG - y_svg) * scale_y

    def _tile_path(self, height_px):
        """One period of the wave, as a filled QPainterPath, in pixels."""
        sx = self.SCALE_X
        path = QtGui.QPainterPath()
        path.moveTo(0, self._y(44))
        path.cubicTo(30*sx, self._y(44), 58*sx, self._y(26), 88*sx, self._y(26))
        path.cubicTo(118*sx, self._y(26), 146*sx, self._y(44), 176*sx, self._y(44))
        path.lineTo(176*sx, height_px)
        path.lineTo(0, height_px)
        path.closeSubpath()
        return path
    ...
    def paintEvent(self, event):
        ...
        r = self.rect()
        p.setPen(QtCore.Qt.NoPen)
        period_px = self.PERIOD_UNITS * self.SCALE_X
        tile = self._tile_path(r.height())
        elapsed = time.monotonic() - self._t0
        for period_s, direction, color, phase_frac, wobble_s, wobble_frac \
                in self._LAYERS:
            # Base linear scroll, full loops of period_px per period_s
            # seconds.
            offset = (elapsed / period_s) * period_px * direction
            # Fixed phase offset -- staggers this layer's starting
            # position instead of all layers beginning in sync at t=0.
            offset += phase_frac * period_px
            # Slow secondary sine wobble, on a period that shares no
            # simple ratio with period_s, so the combined motion doesn't
            # repeat in a short, obviously-mechanical cycle.
            if wobble_s > 0:
                offset += (wobble_frac * period_px *
                          math.sin(2.0 * math.pi * elapsed / wobble_s))
            offset = offset % period_px
            p.setBrush(color)
            # Draw enough tiles to cover the widget width plus one extra
            # on each side so the scrolled offset never exposes a gap.
            n_tiles = int(r.width() / period_px) + 3
            start_x = -period_px + offset
            for i in range(n_tiles):
                p.save()
                p.translate(start_x + i * period_px, r.top())
                p.drawPath(tile)
                p.restore()
        super().paintEvent(event)
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 139)")
    ...
    def _build_gui(self):
        ...
        self._title_wave = WaveHeaderWidget()
        self._title_wave.setFixedHeight(70)
