...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/12 - Version 61)")
...
def main():
    ...
    # Calling tb.start() before app.exec_() causes audio.source to register
    # QSocketNotifiers from a non-Qt thread -- Qt warns and the resulting
    # socket event swallows the first mouse click.  singleShot(0) fires on
    # the very first event-loop iteration so Qt owns the notifiers cleanly.
    QtCore.QTimer.singleShot(0, tb.start)
