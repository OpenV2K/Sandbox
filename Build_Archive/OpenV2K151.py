...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 151)")
    ...
    def _build_gui(self):
        ...
        # Double drop shadow for a sense of height/thickness behind the
        # text, not just one flat offset.  QGraphicsEffect only allows
        # ONE effect per widget (setting a second replaces the first),
        # so this nests two widgets instead: a 1px shadow on the label
        # itself, and a separate 2px shadow on a wrapper widget around
        # it.  Since a graphics effect renders its whole widget's output
        # as one composited image, the outer 2px shadow casts a copy of
        # the ALREADY-shadowed inner content (text + its own 1px shadow)
        # -- producing two visible offset edges rather than one, which
        # reads as a bit of stacked height/thickness behind the letters.
        # Both white, fully opaque, zero blur -- sharp edges throughout.
        _title_shadow_inner = QtWidgets.QGraphicsDropShadowEffect()
        _title_shadow_inner.setColor(QtGui.QColor(255, 255, 255, 255))
        _title_shadow_inner.setOffset(1, 1)
        _title_shadow_inner.setBlurRadius(0)
        title.setGraphicsEffect(_title_shadow_inner)

        _title_shadow_wrap = QtWidgets.QWidget()
        _title_shadow_wrap.setStyleSheet("background: transparent;")
        _tsw_vbox = QtWidgets.QVBoxLayout(_title_shadow_wrap)
        _tsw_vbox.setContentsMargins(0, 0, 0, 0)
        _tsw_vbox.addWidget(title)

        _title_shadow_outer = QtWidgets.QGraphicsDropShadowEffect()
        _title_shadow_outer.setColor(QtGui.QColor(255, 255, 255, 255))
        _title_shadow_outer.setOffset(2, 2)
        _title_shadow_outer.setBlurRadius(0)
        _title_shadow_wrap.setGraphicsEffect(_title_shadow_outer)
        ...
        _title_wave_vbox.addWidget(_title_shadow_wrap)
