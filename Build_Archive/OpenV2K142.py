...
class PulseHeaderWidget(QtWidgets.QWidget):
    """
    Lightweight animated 'boxcar pulse train' background behind the
    OpenV2K title -- replaces the earlier wave effect with a scrolling
    oscilloscope-style digital trace: a flat baseline during silence,
    full-height rectangular pulses at randomised intervals.  Much
    cheaper to render than the wave's antialiased bezier paths -- just
    plain rectangle fills and a couple of lines, no curves, no
    antialiasing needed (sharp edges suit a digital-pulse look anyway).

    Same background area, same width/position (wrapping the title
    label, child renders on top), and same rightward-only scroll
    direction as the wave version it replaces.

    Bitstream: a fixed-length tick sequence generated ONCE at
    construction (not regenerated per frame) with Python's built-in
    random module -- lightweight, no extra dependency.  Each pulse tick
    is followed by a random 1-6 ticks of silence before the next pulse,
    repeated to build a long-enough sequence, then tiled and scrolled
    indefinitely via modulo (same seamless-loop technique the wave used).

    The baseline sits like an underline just beneath the header text
    (same "4px above the bottom edge" position the wave's trough used).
    ...
    set to 0 specifically so this alignment is exact, not approximate
    (a QPainter can never draw outside its own widget's bounds, so this
    was the only way to make "reaches the window's top edge" literally
    true rather than a guess).
    """
    TICK_PX = 8    # was 4 -- wider, so individual pulse rise/fall edges
                   # and the silence gaps between them read clearly
                   # instead of blurring into what looks like one block
    N_TICKS = 200          # pre-generated sequence length (fixed, cheap)
    MIN_SILENCE = 1
    MAX_SILENCE = 6
    BASELINE_Y_PX = 44.0   # moved lower, toward the bottom edge of the
                          # Event Log/Ref buttons.  The 2x2 ref cluster's
                          # own height is confirmed exactly from code
                          # (20px + 2px spacing + 20px = 42px); 44px
                          # accounts for it likely sitting vertically
                          # centred within this taller 46px-tall row.
                          # An estimate -- nudge this value directly if
                          # it doesn't land exactly once rendered.
    LINE_COLOR = QtGui.QColor(220, 220, 220)   # lightest grey from the
                                                # wave palette
    SCROLL_PERIOD_S = 10.0   # seconds for the whole sequence to scroll by
    ...
    def __init__(self, parent=None):
        ...
        self._ticks = self._generate_bitstream()

    def _generate_bitstream(self):
        """One pulse tick, then a random 1-6 ticks of silence, repeated
        until at least N_TICKS long.  Generated once and reused for
        every scroll loop -- no per-frame randomness, no regeneration.

        Deliberately does NOT truncate to exactly N_TICKS: the sequence
        always STARTS on a pulse (first thing the loop below adds), so
        cutting it off at an arbitrary point could land the LAST tick on
        a pulse too -- and since this sequence loops seamlessly for the
        scroll animation, that would put two pulses directly adjacent at
        the wraparound seam with zero silence between them, breaking the
        minimum-silence guarantee right where one loop rolls into the
        next.  Keeping the final (pulse + silence run) unit whole
        instead guarantees the sequence always ends in silence, so the
        wraparound is always safe -- actual length may run slightly past
        N_TICKS as a result, which is fine; paintEvent uses the real
        length, not the N_TICKS target."""
        rng = random.Random(20260816)   # fixed seed: deterministic and
                                        # reproducible run to run
        ticks = []
        while len(ticks) < self.N_TICKS:
            ticks.append(True)
            ticks.extend([False] * rng.randint(self.MIN_SILENCE, self.MAX_SILENCE))
        return ticks

    def paintEvent(self, event):
        p = QtGui.QPainter(self)   # no antialiasing -- sharp rectangle
                                    # edges are correct for this look and
                                    # cheaper to render
        r = self.rect()
        seq_w = len(self._ticks) * self.TICK_PX
        elapsed = time.monotonic() - self._t0
        offset = (elapsed / self.SCROLL_PERIOD_S) * seq_w
        offset = offset % seq_w

        # Baseline, drawn once across the full width -- the "underline"
        # beneath the header text, silence ticks just ride along it.
        p.setPen(QtGui.QPen(self.LINE_COLOR, 2))
        p.drawLine(r.left(), int(self.BASELINE_Y_PX),
                  r.right(), int(self.BASELINE_Y_PX))

        # Pulse rectangles: baseline down at BASELINE_Y_PX, top edge at
        # this widget's own y=0 -- which is also the window's actual top
        # edge (see class docstring).
        p.setPen(QtCore.Qt.NoPen)
        ...
        pulse_h = int(self.BASELINE_Y_PX)
        for rep in range(n_repeats):
            base_x = start_x + rep * seq_w
            if base_x > r.width() or base_x + seq_w < 0:
                continue   # this whole repeat is off-screen -- skip it
            for i, is_pulse in enumerate(self._ticks):
                if not is_pulse:
                    continue
                tick_x = base_x + i * self.TICK_PX
                if tick_x + self.TICK_PX < 0 or tick_x > r.width():
                    continue   # this tick is off-screen -- skip it
                p.drawRect(QtCore.QRect(int(tick_x), 0,
                                        self.TICK_PX, pulse_h))
        super().paintEvent(event)
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 142)")
