...
class HorizontalSegmentedMeter(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        total_gap = gap * (n - 1)
        seg_w = (r.width() - total_gap) / float(n)
        lit = int(round(((self._value_db + 60.0) / 60.0) * n))
        p.setPen(QtCore.Qt.NoPen)
        for i in range(n):
            seg_num = i + 1   # 1 = leftmost
            left = r.left() + (seg_num - 1) * (seg_w + gap)
            seg_rect = QtCore.QRectF(left, r.top(), seg_w, r.height())
            if seg_num <= lit:
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 134)")
