...
class WaveHeaderWidget(QtWidgets.QWidget):
    ...
    """
    # One wave period in the path's own coordinate units (SVG y: 26=peak,
    # 44=trough).  Curve control points decoded directly from the
    # original "M-160 44c30 0 58-18 88-18s58 18 88 18..." SVG path data.
    PERIOD_UNITS = 176.0
    PEAK_Y = 26.0
    TROUGH_Y = 44.0
    ...
    _LAYERS = [
        (10.0, +1, QtGui.QColor(232, 232, 232), 255),
        ( 8.0, -1, QtGui.QColor(216, 216, 216), 255),
        ( 6.0, +1, QtGui.QColor(200, 200, 200), 255),
        ( 4.0, -1, QtGui.QColor(184, 184, 184), 255),
    ]

    def __init__(self, parent=None):
        super().__init__(parent)
        self._scale = 0.5   # SVG units -> pixels
    ...
    def _tile_path(self, height_px):
        ...
        path.moveTo(0, self.TROUGH_Y * s)
        path.cubicTo(30*s, 44*s, 58*s, 26*s, 88*s, 26*s)
        path.cubicTo(118*s, 26*s, 146*s, 44*s, 176*s, 44*s)
        path.lineTo(176*s, height_px)
    ...
    def paintEvent(self, event):
        ...
        r = self.rect()
        p.setPen(QtCore.Qt.NoPen)
        period_px = self.PERIOD_UNITS * self._scale
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 136)")
