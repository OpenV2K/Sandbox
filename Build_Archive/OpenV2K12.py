...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 11)")
    ...
    def _build_blocks(self):
        ...
        # 3. Source selector: input 0 = mic, input 1 = eSpeak
        self.src_selector = blocks.selector(gr.sizeof_float, 2, 1)
        self.src_selector.set_input_index(0)
    ...
    def _connect_blocks(self):
        # Source selector fan-ins
        self.connect(self.audio_src,  (self.src_selector, 0))
        self.connect(self.espeak_src, (self.src_selector, 1))

        # Level probe always sees raw mic
        self.connect(self.audio_src, self.level_probe)

        # Main processing chain
        self.connect(self.src_selector, self.mute_gate)
    ...
    def _cb_mute(self, muted):
        ...
        if muted:
            ...
            # Switching mic LIVE also restores mic as audio source
            self.src_selector.set_input_index(0)
    ...
    def _cb_generate_espeak(self):
        ...
        # Lock graph, swap espeak source with new file, switch selector
        self.lock()
        self.disconnect(self.espeak_src, (self.src_selector, 1))
        self.espeak_src = blocks.wavfile_source(self.ESPEAK_WAV, True)
        self.connect(self.espeak_src, (self.src_selector, 1))
        self.src_selector.set_input_index(1)
        self.unlock()
