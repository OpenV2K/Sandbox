...
_MBROLA_ACCENTS = {
    'en': [('British',    'en1'), ('American',  'us1')],
    'es': [('Spain',      'es1'), ('Mexico',    'mx1'), ('Venezuela', 'vz1')],
    'pt': [('Portugal',   'pt1'), ('Brazil',    'br1')],
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/12 - Version 82)")
    ...
    def _populate_accent_combo(self):
        ...
        accent options (if any), and enable it only when there's an actual
        choice to make (2+ accents).  For every other language it shows a
        single disabled "--" placeholder.  Called at construction and again
        whenever the language selector changes.
        """
        ...
        if accents and len(accents) > 1:
            ...
            self._accent_combo.addItem("--", None)
            self._accent_combo.setEnabled(False)
            self._accent_combo.setToolTip(
                "No alternate MBROLA accent available for this language.")
    ...
    def _cb_accent_changed(self, index):
        ...
        self._log("MBROLA accent: {} ({})".format(
    ...
    def _build_gui(self):
        ...
        # Accent combobox -- sits right next to the language combobox.
        # Only enabled when the selected language has more than one genuine
        # MBROLA national/regional accent (English, Spanish, Portuguese,
        # French).  Disabled + shows "--" for every other language, since
        # there's no accent choice to make.
        self._ACCENT_W = 50
        ...
        # Split the button's own width between lang_combo and accent_combo
        # so the row together still roughly matches the button's footprint
        # (keeps the title from being squeezed more than before).
        _lang_w = max(40, self._log_btn.sizeHint().width()
                     - self._ACCENT_W - 3)
        self._lang_combo.setMaximumWidth(_lang_w)

        self._populate_accent_combo()   # sets initial enabled state + items
