...
class SwapPanel(QtWidgets.QWidget):
    ...
    Two different feather treatments, both only drawn while dimmed:

    - Top/bottom edges: a tight N-pixel step gradient (nested inset
      rects, each more opaque than the last).  A QSS radial gradient
      can't give a uniform pixel-width falloff on all sides of a
      rectangle -- edge-midpoints and corners sit at different distances
      from the center -- so this paints it directly instead.

    - The edge facing the SwapButton (set via divider_side): MOST of the
      panel stays solid, uniform opaque grey -- no gradient at all.  Only
      a short band right at the edge nearest the button fades, and that
      fade's mathematical endpoint (full transparency) is placed PAST the
      panel's own boundary, roughly at the SwapButton's own centre line --
      even though nothing is actually drawn out there (a widget can't
      paint outside its own geometry).  The practical effect: the panel's
      own edge pixel is only PARTWAY faded, not fully transparent, so the
      fade visually reads as continuing on behind the button instead of
      hard-stopping at the panel boundary, without ever overlapping or
      obstructing the button itself.
    """
    _TRANSITION_W = 30    # how far INTO the panel the fade zone extends,
                          # measured from the edge nearest the button
    _BUTTON_HALF_W = 22   # SwapButton.W // 2 -- how far PAST the panel's
                          # edge the gradient's zero-alpha point sits
    ...
    def paintEvent(self, event):
        if self._dim_active:
            ...
            r = self.rect()
            c = self._dim_color
            max_a = self._max_alpha
            opaque = QtGui.QColor(c.red(), c.green(), c.blue(), max_a)
            transparent = QtGui.QColor(c.red(), c.green(), c.blue(), 0)

            # Solid opaque fill everywhere first -- most of the panel has
            # NO gradient at all.
            p.fillRect(r, opaque)

            # Short fade zone right at the edge nearest the SwapButton.
            # Gradient's endpoints extend past the panel's own boundary on
            # the button side, so only a PARTIAL fade is ever visible --
            # full transparency is never actually reached within this
            # widget's own paintable area.
            grad = QtGui.QLinearGradient()
            if self._divider_side == 'right':
                fade_start_x = r.right() - self._TRANSITION_W
                fade_end_x   = r.right() + self._BUTTON_HALF_W
                transition_rect = QtCore.QRect(
            ...
            p.fillRect(transition_rect, grad)

            # Tight top/bottom feather (unchanged from before) -- pixel
            # exactly d px from the top or bottom edge gets the same
            # alpha regardless of which side, layered on top.
            p.setPen(QtCore.Qt.NoPen)
            n = self._feather_px
            for i in range(n):
                alpha = int(max_a * (i + 1) / float(n))
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/14 - Version 112)")
