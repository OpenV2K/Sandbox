...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/12 - Version 62)")
    ...
    def _start_suppressed(self):
        """Start the GR flow graph with raw stderr briefly sent to /dev/null.
        QSocketNotifier warning is emitted via C++ fprintf -- not catchable by
        Python message handlers -- so fd-level redirection is the only fix."""
        import os
        try:
            saved = os.dup(2)
            ...
            self.start()          # always start even if fd redirect fails
        self._prime_mbrola_async()

    def _prime_mbrola_async(self):
        """Warm up MBROLA in a background thread immediately on startup.
        The binary has a cold-start delay (loading voice data) that makes the
        very first synthesis produce near-silence.  This throw-away call
        absorbs the delay before the user clicks Generate Voice."""
        if not _MBROLA_OK:
            return
        def _prime():
            import tempfile
            for cmd in ['espeak-ng', 'espeak']:
                try:
                    with tempfile.NamedTemporaryFile(suffix='.wav',
                                                    delete=True) as f:
                        subprocess.run([cmd, '-v', 'mb-en1', '-a', '0',
                                        '-w', f.name, 'a'],
...
def main():
    ...
    _install_qt_msg_filter()   # suppress GR QSocketNotifier warning
    tb = OpenV2K(); tb.show()
