...
class SwapPanel(QtWidgets.QWidget):
    ...
    The panel itself is 100% solid grey right up to its own edges, with
    no left/right fade at all -- the fade toward the shared SwapButton
    lives entirely in SwapButton's own paintEvent instead (see that
    class), so it renders behind the button's clickable circle rather
    than inside the panel.  Solid, fully-opaque colours throughout for
    the top/bottom feather -- no alpha transparency, no compositing
    ambiguity.
    """
    ...
    def __init__(self, parent=None, divider_side='right'):
        ...
        self._divider_side = divider_side   # kept for reference/back-compat;
                                             # no longer used for painting
    ...
    def paintEvent(self, event):
        if self._dim_active:
            ...
            # Solid grey, everywhere, right up to every edge -- no fade.
            p.fillRect(r, c)

            # Tight top/bottom feather only -- solid interpolated colours.
...
class SwapButton(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self._left_dimmed:
            ...
            p.fillRect(QtCore.QRect(r.left(), r.top(), cx - r.left(), r.height()), grad)
        if self._right_dimmed:
            grad = QtGui.QLinearGradient(cx, 0, r.right(), 0)
            grad.setColorAt(0.0, bg); grad.setColorAt(1.0, c)
            p.fillRect(QtCore.QRect(cx, r.top(), r.right() - cx, r.height()), grad)
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/14 - Version 114)")
