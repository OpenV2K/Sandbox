...
class WaveHeaderWidget(QtWidgets.QWidget):
    ...
    All 4 layers scroll rightward only, at their own constant speed
    (10s/8s/6s/4s per full loop) -- no direction reversal, no
    oscillation.  The parallax depth illusion comes entirely from those
    4 different constant speeds, not from any layer changing direction
    or wobbling.
    """
    ...
    AMPLITUDE_PX = 36.0    # header height reverted to its original 46px
                          # (was briefly grown to 70px, undone per request)
                          # so amplitude is capped at what actually fits
                          # without clipping the peak -- still ~3x the
                          # original 11.7px, just not the full 50px that
                          # needed the taller widget.
    TROUGH_Y_PX = 42.0    # explicit position: 4px above the bottom edge,
                          # same rule as always -- unaffected by the
                          # amplitude/wavelength changes above

    # (period_seconds, grey_color) -- back to front.  Back to 4 layers.
    # No direction reversal and no wobble -- every layer scrolls in the
    # SAME direction (right) at its own constant speed; the parallax
    # effect comes entirely from those 4 different speeds (10s/8s/6s/4s),
    # not from any layer changing direction or oscillating.  Lightened
    # further per request -- the two darker greys from the previous
    # attempt (175, 140) are now both notably lighter (190, 175).
    _LAYERS = [
        (10.0, QtGui.QColor(220, 220, 220)),
        ( 8.0, QtGui.QColor(205, 205, 205)),
        ( 6.0, QtGui.QColor(190, 190, 190)),
        ( 4.0, QtGui.QColor(175, 175, 175)),
    ]
    ...
    def paintEvent(self, event):
        ...
        for period_s, color in self._LAYERS:
            # Plain linear scroll, always rightward, full loops of
            # period_px per period_s seconds -- the parallax look comes
            # entirely from each layer's own constant speed being
            # different, not from any direction change or oscillation.
            offset = (elapsed / period_s) * period_px
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 140)")
    ...
    def _build_gui(self):
        ...
        self._title_wave.setFixedHeight(46)
