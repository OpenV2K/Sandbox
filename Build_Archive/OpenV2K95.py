...
'en': {
    ...
    'power_calc_summary':
        'Calculates the hypothetical energy per pulse and total session '
        'energy a high-power microwave amplifier could deliver, using the '
        'current pulse width and pulse count. Purely illustrative -- not '
        'connected to this app\'s actual HackRF output. 2kW is a round '
        'reference point near the FCC amateur radio power ceiling '
        '(47 CFR 97.313 sets 1500W PEP as the general maximum, for all '
        'license classes on most bands). 4kW reflects the rated maximum '
        'output of the EXODUS Advanced Communications AMP2043P-4KW '
        'microwave amplifier (1-2 GHz), per its published spec sheet.',
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/12 - Version 95)")
    ...
    def _retranslate_ui(self):
        ...
        self._hdr_power_calc.set_label_text(_tr("power_calculation"))
    ...
    def _build_gui(self):
        ...
        # =====================================================================
        # Power Calculation -- its own top-level section, same blue-gradient
        # SectionHeader treatment as Audio Input / Signal Processing / Output.
        # Moved out of Signal Processing entirely for room to breathe and
        # bigger, more readable fonts.
        # =====================================================================
        self._hdr_power_calc = SectionHeader(_tr("power_calculation"))
        vbox.addWidget(self._hdr_power_calc)
        vbox.addSpacing(4)

        pc_section = QtWidgets.QWidget()
        pc_section_vbox = QtWidgets.QVBoxLayout(pc_section)
        pc_section_vbox.setContentsMargins(10, 6, 10, 8)
        pc_section_vbox.setSpacing(6)

        # Summary -- explains what this section calculates and why 2kW/4kW
        # were chosen as the two illustrative reference tiers.
        ...
        # Reset link -- own row now that the redundant internal title (the
        # SectionHeader already says "Power Calculation") is gone.
        ...
        self._power_reset_lbl = QtWidgets.QLabel(
            "<a href='#' style='color:#2a6ebb; text-decoration:none;'>{}</a>"
            .format(_tr("power_reset")))
        self._power_reset_lbl.setStyleSheet("font-size:10pt;")
        self._power_reset_lbl.linkActivated.connect(self._cb_power_reset)
        reset_row.addWidget(self._power_reset_lbl)
        pc_section_vbox.addLayout(reset_row)

        # Bordered box with the two live data rows -- bigger font now that
        # there's a full-width section to work with instead of a cramped
        # grid rectangle.
        self._power_calc_box = QtWidgets.QGroupBox()
        self._power_calc_box.setStyleSheet("QGroupBox { font-size:10pt; }")
        pc_vbox = QtWidgets.QVBoxLayout(self._power_calc_box)
        pc_vbox.setContentsMargins(10, 10, 10, 10); pc_vbox.setSpacing(8)
        ...
        self._power_perpulse_lbl.setStyleSheet("font-size:11px; color:#333;")
        ...
        # Fixed gap (not stretch) -- computed so the TX button's top lands
        # at the same fixed vertical offset as Record IQ's top in the Save
        # panel, regardless of how the two panels' overall heights get
        # equalized by the shared HBoxLayout row.
        tx_vbox.addSpacing(31)
