...
def detect_hackrf():
    try:
        ...
        if 'HackRF' in output and result.returncode == 0:
            ...
            # Strategy 1: "(API:" marker is unique to the firmware version line
            ...
            # Strategy 2: any line with "firmware" and ":" not from the tool itself
            if firmware is None:
                for l in lines:
                    ll = l.lower()
                    if ('firmware' in ll and ':' in l
                            and 'hackrf_info' not in ll
                            and 'libhackrf'   not in ll):
                        m = re.search(r'(\d+\.\d+[\.\d]*)', l.split(':', 1)[1])
                        if m:
                            firmware = m.group(1); break

            # Strategy 3: firmware= style
            if firmware is None:
                for l in lines:
                    if 'firmware' in l.lower() and '=' in l:
                        m = re.search(r'(\d+\.\d+[\.\d]*)', l.split('=', 1)[1])
                        if m:
                            firmware = m.group(1); break
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 23)")
