...
class SwapPanel(QtWidgets.QWidget):
    """
    A panel that can show a feathered grey overlay when 'inactive' (used
    for the Mic/eSpeak input-source swap panels and the TX/Save output
    panels).

    A QSS radial gradient can't give a UNIFORM pixel-width falloff on all
    four sides of a rectangle -- a radial gradient's distance-from-center
    differs between edge-midpoints and corners on anything that isn't a
    perfect square, so the "feather width" ends up inconsistent around
    the shape.  This paints the overlay directly instead: N nested rects,
    each inset by one more pixel than the last and progressively more
    opaque, so a pixel exactly d px from ANY edge gets exactly the same
    alpha regardless of which edge (or corner) it's near.
    """
    ...
    def paintEvent(self, event):
        if self._dim_active:
            ...
            r = self.rect()
            n, c, max_a = self._feather_px, self._dim_color, self._max_alpha
            for i in range(n):
                alpha = int(max_a * (i + 1) / float(n))
                p.setBrush(QtGui.QColor(c.red(), c.green(), c.blue(), alpha))
                p.drawRect(r.adjusted(i, i, -i, -i))
            core = r.adjusted(n, n, -n, -n)
            p.setBrush(QtGui.QColor(c.red(), c.green(), c.blue(), max_a))
            p.drawRect(core)
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/12 - Version 108)")
    ...
    def _build_gui(self):
        ...
        # Bordered box: 3 data rows on the left, a large rounded light-blue
        # Reset button filling the right end.  Same translated description
        # as the header above, shown as a tooltip on hover.
        self._power_calc_box = QtWidgets.QGroupBox()
        self._power_calc_box.setToolTip(_tr("power_calc_summary"))
        self._power_calc_box.setStyleSheet(
            "QGroupBox { font-size:9pt; margin-top:0px; }")
