...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/14 - Version 124)")
    ...
    def _build_gui(self):
        ...
        dc_vbox.addSpacing(4)   # whitespace above the Live/Peak labels,
                                # which now sit at the very top of the section

        # Peak and Live meters side by side (Peak now on the left, Live on
        # the right).  Both use SegmentedMeter (see class above) instead
        # of QProgressBar, which fixes a real bug where the old gradient's
        # colour stops were scoped to the filled chunk's own geometry
        # rather than the bar's absolute value range -- meaning even a
        # low reading could show red right at its own small top edge.
        ...
        # -- Peak column (left): label, numeric readout, own meter --
        ...
        peak_bar_row.addStretch()
        peak_col.addLayout(peak_bar_row, 1)   # stretch -- meter fills
                                               # remaining height, directly
                                               # under the readout, restoring
                                               # the original taller size
        meters_row.addLayout(peak_col, 1)
        meters_row.addSpacing(10)   # 10px closer together than before

        # -- Live column (right): existing readout kept --
        ...
        live_bar_row.addStretch()
        live_col.addLayout(live_bar_row, 1)   # stretch -- same as Peak
