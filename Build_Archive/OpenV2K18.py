...
# =============================================================================
#  HackRF hardware detection  (regex firmware version extraction)
# =============================================================================
...
def detect_hackrf():
    try:
        ...
        if 'HackRF' in output and result.returncode == 0:
            ...
            firmware = None
            # Pass 1: look for any line containing 'firmware', extract X.X.X
            for l in lines:
                if 'firmware' in l.lower():
                    m = re.search(r'(\d+\.\d+[\.\d]*)', l)
                    if m:
                        firmware = m.group(1)
                        break
            # Pass 2: fallback -- any version-like number anywhere in output
            if firmware is None:
                m = re.search(r'(\d+\.\d+[\.\d]*)', output)
                firmware = m.group(1) if m else 'see hackrf_info'
...
class LabelledSlider(QtWidgets.QWidget):
    def __init__(self, label, lo, hi, step, default,
        ...
        row.addSpacing(20); row.addWidget(self._readout)
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    # Colour applied to the background of a disabled panel
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 18)")
    ...
    # =========================================================================
    #  Panel active/inactive helper
    #  Sets background colour + enabled state in one call.
    # =========================================================================

    def _set_panel_active(self, panel, active):
        """Enable panel with default background, or disable with darker grey."""
        ...
        if active:
            ...
            pal.setColor(QtGui.QPalette.Window, self._DISABLED_BG)
    ...
    def _build_gui(self):
        ...
        # Left margin ~20px (~0.5cm) from VLine; content area ~87px
        dc_panel = QtWidgets.QWidget()
        dc_vbox  = QtWidgets.QVBoxLayout(dc_panel)
        dc_vbox.setContentsMargins(20, 2, 4, 6)   # 20px left, tight top
        dc_vbox.setSpacing(0)

        # Percentage readout -- black bold, at top
        ...
        # Vertical bar (fills remaining space, green->amber->red)
        ...
        self._dc_vbar.setStyleSheet(
            ...
            "); border-radius:3px; }")
        dc_vbox.addWidget(self._dc_vbar, 1)
        dc_vbox.addSpacing(4)

        # Short summary between bar and label
        ...
        # One-line label at bottom (no word wrap)
        ...
        # =====================================================================
        # Apply initial disabled states (dark bg + greyed content)
        # =====================================================================
        self._set_panel_active(self._mic_panel,  False)  # eSpeak is default
        self._set_panel_active(self._es_panel,   True)
        self._set_panel_active(self._tx_panel,   False)  # Save is default
    ...
    def _build_blocks(self):
        ...
        self.mic_gate    = blocks.multiply_const_ff(0.0)   # eSpeak default
    ...
    def _cb_audio_swap(self):
        ...
        if self._audio_left_active:
            # Mic becomes active -- always start MUTED
            ...
            self._btn_mute.setStyleSheet(self._style_red())
            self.mute_gate.set_k(0.0)
            self._muted = True
        else:
            # eSpeak becomes active
    ...
    def _cb_output_swap(self):
        ...
        if self._output_left_active:
            # HackRF TX becomes active -- re-detect and always start DISABLED
