...
class WaveHeaderWidget(QtWidgets.QWidget):
    """
    Animated 4-layer parallax 'waves divider' background, recreating a
    popular CSS/SVG effect: one repeating wave shape, drawn 4 times at
    different scroll speeds/directions, layered to create a drifting
    ocean look.  Paints in its OWN paintEvent -- which Qt renders BEFORE
    child widgets -- so any child widget placed inside it (e.g. the
    OpenV2K title label) sits on top, unobstructed.  Same "paint behind,
    let children render on top" technique already used for SwapButton's
    domed highlight.
    ...
    scrolled via a QTimer.  Greyscale fills (this app's light background
    doesn't suit the original's navy/pink/blue palette) -- lightest at
    the front (closest) layer, progressively darker toward the back,
    same "brightest = closest" logic the original used with white on top.
    ...
    -- needed to satisfy "taller crests" and "trough N px from the
    bottom edge" as separate, independently-tunable requirements.
    """
    ...
    SCALE_X = 0.75    # was 0.5 -- longer wavelength (132px period vs 88px)
    AMPLITUDE_PX = 11.7   # was 9.0px -- 30% taller crests
    TROUGH_Y_PX = 42.0    # explicit position: 4px above the 46px-tall
                          # widget's bottom edge

    # (period_seconds, direction, grey_color, opacity) -- back to front,
    # matching the original's parallax1..4 speeds (10s/8s/6s/4s) and
    # alternating scroll direction.  Lightest/most-opaque layer last
    # (frontmost), mirroring the original's white top layer.
    #
    # The widget's own background is fixed explicitly (see __init__)
    # rather than left as Qt's default system palette -- that default
    # varies by OS/theme (often ~#f0f0f0 or lighter) and isn't something
    # this code can see or verify, so the first lightened attempt picked
    # colours too close to whatever that turned out to be, making the
    # waves nearly invisible.  Setting a KNOWN background and choosing
    # wave colours with guaranteed contrast against it removes that
    # guesswork entirely.
    _BG_COLOR = QtGui.QColor(255, 255, 255)   # explicit white background
    _LAYERS = [
        (10.0, +1, QtGui.QColor(225, 225, 225), 255),
        ( 8.0, -1, QtGui.QColor(213, 213, 213), 255),
        ( 6.0, +1, QtGui.QColor(201, 201, 201), 255),
        ( 4.0, -1, QtGui.QColor(189, 189, 189), 255),
    ]
    ...
    def paintEvent(self, event):
        ...
        p.setRenderHint(QtGui.QPainter.Antialiasing)
        r = self.rect()
        p.fillRect(r, self._BG_COLOR)   # explicit, known background --
                                        # drawn first so it's guaranteed
                                        # to sit BEHIND the wave layers,
                                        # not fight with Qt's own default
                                        # palette/autofill timing
        p.setPen(QtCore.Qt.NoPen)
        period_px = self.PERIOD_UNITS * self.SCALE_X
        tile = self._tile_path(r.height())
        elapsed = time.monotonic() - self._t0
        for period_s, direction, color, alpha in self._LAYERS:
            # Full loops of period_px per period_s seconds, wrapped with
            # modulo for a seamless infinite scroll in either direction.
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 138)")
