...
# =============================================================================
#  Section Header (blue gradient, centred bold text, white shadow)
# =============================================================================
...
class SectionHeader(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        pen_rule = QtGui.QPen(QtGui.QColor("#888"), 1)
        p.setPen(pen_rule)
        p.drawLine(0, 0, w, 0)
        p.drawLine(0, self.height() - 1, w, self.height() - 1)
        text_x = (w - self._tw) // 2; text_y = gap + self._asc
        p.setFont(self._font)
        p.setPen(QtGui.QColor("white"))
        p.drawText(text_x + 1, text_y + 1, self._text)
...
# =============================================================================
#  Log Tab Button (vertical, right edge of window)
# =============================================================================

class LogTabButton(QtWidgets.QAbstractButton):
    """20px-wide vertical toggle button that reads 'LOG' rotated 90 degrees."""
    ...
    def sizeHint(self):
        return QtCore.QSize(20, 200)

    def paintEvent(self, event):
        p   = QtGui.QPainter(self)
        bg  = QtGui.QColor("#2a6ebb") if self.isChecked() else QtGui.QColor("#555")
        p.fillRect(self.rect(), bg)
        p.setPen(QtGui.QColor("white"))
        f = QtGui.QFont("Monospace", 7); f.setBold(True)
        p.setFont(f)
        # Rotate text so it reads top-to-bottom
        p.translate(self.width(), 0)
        p.rotate(90)
        p.drawText(0, 0, self.height(), self.width(),
                   QtCore.Qt.AlignCenter, "< LOG")


# =============================================================================
#  Audio level meter  (full-width bar; readout managed externally)
# =============================================================================
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 28)")
        self.setFixedWidth(600)    # +20px for LOG tab button
        ...
        self._init_log_drawer()   # must come before _build_gui
        self._build_gui()
        self._build_blocks()
    ...
    def _init_log_drawer(self):
        self._log_drawer = QtWidgets.QWidget()
        self._log_drawer.setWindowFlags(QtCore.Qt.FramelessWindowHint)
        self._log_drawer.setWindowTitle("OpenV2K Log")

        dl = QtWidgets.QVBoxLayout(self._log_drawer)
        ...
        self._log_text.setStyleSheet(
            "QTextEdit { background-color:black; color:white; border:none; }")
        dl.addWidget(self._log_text)

    def _format_log(self, msg, in_progress=False):
        now    = datetime.datetime.now()
        ms     = now.microsecond // 1000
        ts     = now.strftime("%H:%M:%S.") + "{:03d}".format(ms)
        suffix = "   ..." if in_progress else "."
        return "[{}] {}{}".format(ts, msg, suffix)

    def _log(self, msg, in_progress=False):
        """Append a timestamped log entry (call from main thread only)."""
        line = self._format_log(msg, in_progress)
        self._log_text.append(line)
        sb = self._log_text.verticalScrollBar()
        sb.setValue(sb.maximum())

    def _log_ts(self, msg, in_progress=False):
        """Thread-safe log -- can be called from background threads."""
        line = self._format_log(msg, in_progress)
    ...
    def _toggle_log_drawer(self, checked):
        if checked:
            self._reposition_drawer()
            self._log_drawer.resize(660, self.height())
            self._log_drawer.show()
        else:
            self._log_drawer.hide()

    def _reposition_drawer(self):
        pos = self.mapToGlobal(QtCore.QPoint(self.width(), 0))
        self._log_drawer.move(pos)

    def moveEvent(self, event):
        super().moveEvent(event)
        if hasattr(self, '_log_drawer') and self._log_drawer.isVisible():
            self._reposition_drawer()
    ...
    def _build_gui(self):
        ...
        # Outer horizontal layout: content + LOG tab button
        ...
        h_outer.addWidget(content_w, 1)

        self._log_tab = LogTabButton()
        self._log_tab.toggled.connect(self._toggle_log_drawer)
        h_outer.addWidget(self._log_tab)
        ...
        # "Live Microphone" same style as eSpeak title (bold, default size)
        ...
        # Description: same width as button (both fill mic_panel width)
        ...
        # dB readout ABOVE the meter (moved up from v27 right-side position)
        ...
        self._level_db_lbl.setStyleSheet("color: #27ae60; margin-bottom:0px;")
        mic_vbox.addWidget(self._level_db_lbl)

        # Full-width meter (no readout inside)
        ...
        self._char_counter.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        ...
        # Multi-line text input (word-wrap, letters+spaces, 140 chars max)
        ...
        dc_lbl.setFont(font_dc)
        dc_lbl.setStyleSheet("color: black;")
        ...
        # Header: Transmitter title + HackRF detection (right-justified)
        ...
        # Stretch pushes frequency/power/TX down ~2cm from licensing text
        ...
        # TX button bottom edge is the bottom of this panel
        ...
        save_vbox.setContentsMargins(4, 4, 0, 5)   # 5px bottom -> checkbox 5px from GUI edge
        ...
        # Description: fills full panel width, larger font
        ...
        # Stretch pushes Record IQ + checkbox to bottom
        save_vbox.addStretch(1)
        save_vbox.addSpacing(8)   # extra whitespace between description and button
        ...
        self._btn_record.setSizePolicy(QtWidgets.QSizePolicy.Expanding,
        ...
        self._chk_waterfall = QtWidgets.QCheckBox(
            "Generate Waterfall Graph Image")
        ...
        # Checkbox bottom is 5px from GUI bottom (via setContentsMargins bottom=5)
    ...
    def _update_displays(self):
        if self._audio_left_active:
            ...
            colour = ("#e74c3c" if db > -6 else
                      "#f39c12" if db > -18 else "#27ae60")
            self._level_db_lbl.setText("{:+.1f} dB".format(db))
            self._level_db_lbl.setStyleSheet(
                "color: {}; font-family:Monospace; font-size:8px;".format(colour))
    ...
    def _on_espeak_text_changed(self):
        """Sanitise eSpeak input: letters+spaces only, max 140 chars."""
        ...
        self._btn_generate.setEnabled(
            len(stripped) > 0 and stripped.lower() != 'hello world')
    ...
    def _cb_generate_espeak(self):
        ...
        if cmd_found is None:
            ...
            self._log("eSpeak not installed"); return
        ...
        if r.returncode != 0:
            ...
            self._log("eSpeak generation failed"); return
        ...
        try:
            ...
            self._log("WAV read error"); return
        ...
        # Silence padding: reduces average duty cycle from ~15% to ~2-4%
    ...
    def _generate_waterfall(self, iq_path):
        try:
            ...
            if len(data) < 1024:
                ...
                self._log_ts("Waterfall: recording too short"); return
            ...
            msg = "Waterfall saved: {}".format(os.path.basename(png_path))
            self._set_wf_status("Saved: {}".format(os.path.basename(png_path)))
            self._log_ts(msg)
        except Exception as e:
            self._set_wf_status("Error: {}".format(e))
            self._log_ts("Waterfall error: {}".format(e))
