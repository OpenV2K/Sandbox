...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/12 - Version 104)")
    ...
    def _set_panel_active(self, panel, active):
        ...
        panel.setObjectName("swapPanel")
        if active:
            panel.setAutoFillBackground(False)
            panel.setStyleSheet("#swapPanel { background: transparent; }")
        else:
            # Feathered radial gradient instead of a hard-edged flat fill --
            # solid dark grey through the middle, smoothly fading to fully
            # transparent (matching whatever's behind it) near the edges.
            bg = self._DISABLED_BG
            panel.setAutoFillBackground(False)
            panel.setStyleSheet(
                "#swapPanel {{ background: qradialgradient(cx:0.5, cy:0.5,"
                " radius:0.85, fx:0.5, fy:0.5,"
                " stop:0 rgba({r},{g},{b},225),"
                " stop:0.6 rgba({r},{g},{b},225),"
                " stop:1 rgba({r},{g},{b},0)); }}"
                .format(r=bg.red(), g=bg.green(), b=bg.blue()))
    ...
    def _build_gui(self):
        ...
        tx_hdr = QtWidgets.QLabel(
            "<span style='font-weight:bold; font-size:11px; color:black;'>"
            "SDR Transmitter</span>")
        ...
        # TX button now comes right after the combos (swapped with the
        # license text below) -- lines up much closer with Record IQ's
        # position in the Save panel, which also sits right after its
        # description with minimal gap.
        tx_vbox.addSpacing(0)
        ...
        # License text now sits below the button.
