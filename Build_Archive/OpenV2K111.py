...
class SwapPanel(QtWidgets.QWidget):
    ...
    which sit side by side in pairs with a shared vertical divider
    between them).
    ...
    - The edge facing the paired panel (set via divider_side): a much
      slower, wider linear gradient spanning the panel's full width,
      fully transparent (background colour) right at the shared divider
      and fully opaque dark grey by the panel's own outer edge -- so the
      seam between the active and disabled panel isn't a hard line.
    """
    ...
    def paintEvent(self, event):
        if self._dim_active:
            ...
            # Slow horizontal gradient toward the divider -- covers the
            # whole panel width, drawn first so the tight top/bottom
            # feather (below) can lightly refine the very edges on top.
            ...
            if self._divider_side == 'right':
                grad.setStart(r.right(), 0); grad.setFinalStop(r.left(), 0)
            else:
                grad.setStart(r.left(), 0); grad.setFinalStop(r.right(), 0)
            ...
            # alpha regardless of which side, layered on top of the
            # horizontal gradient above.
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/14 - Version 111)")
    ...
    def _build_gui(self):
        ...
        self._mic_panel = SwapPanel(divider_side='left')  # left member of its pair -- swapped, gradient was reversed
        ...
        self._es_panel = SwapPanel(divider_side='right')  # right member of its pair -- swapped, gradient was reversed
        ...
        self._tx_panel = SwapPanel(divider_side='left')  # left member of its pair -- swapped, gradient was reversed
        ...
        self._save_panel = SwapPanel(divider_side='right')  # right member of its pair -- swapped, gradient was reversed
