...
class ChamferedButton(QtWidgets.QPushButton):
    """
    A QPushButton whose four corners are cut at a literal 45-degree
    chamfer (chiseled/faceted edge) rather than rounded.  Achieved with a
    QRegion mask recomputed on every resize -- Qt clips the widget's
    visible AND clickable area to that region, but still renders the
    normal QSS-styled background/text/border inside it first, so all of
    the existing colour-changing stylesheet logic (_style_red/_style_green,
    hover, disabled, checked) keeps working completely unchanged.  Only
    the widget's outer SHAPE is affected.
    """
    CHAMFER = 10   # px cut from each corner

    def resizeEvent(self, event):
        super().resizeEvent(event)
        w, h = self.width(), self.height()
        c = max(0, min(self.CHAMFER, w // 2, h // 2))
        poly = QtGui.QPolygon([
            QtCore.QPoint(c, 0),     QtCore.QPoint(w - c, 0),
            QtCore.QPoint(w, c),     QtCore.QPoint(w, h - c),
            QtCore.QPoint(w - c, h), QtCore.QPoint(c, h),
            QtCore.QPoint(0, h - c), QtCore.QPoint(0, c),
        ])
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/8/14 - Version 120)")
    ...
    def _style_green():
        return ("QPushButton { background-color:#27ae60; color:white;"
                ...
                " background-color:#1e8449; border-color:#1e8449; }"
    ...
    def _style_red():
        return ("QPushButton { background-color:#c0392b; color:white;"
                " border-style:outset; border-width:2px;"
                " border-color:#c0392b; font-weight:bold; }"
                "QPushButton:hover { background-color:#e74c3c;"
                " border-color:#e74c3c; }"
                "QPushButton:pressed { border-style:inset;"
                " background-color:#922b21; border-color:#922b21; }"
                "QPushButton:disabled { background-color:#555555;"
                " color:#999999; border-color:#555555; }")
    ...
    def _build_gui(self):
        ...
        self._btn_mute = ChamferedButton(_tr("mic_muted"))
        ...
        self._btn_generate = ChamferedButton(_tr("generate_voice"))
        ...
        self._btn_tx = ChamferedButton(_tr("tx_disabled"))
        ...
        self._btn_record = ChamferedButton(_tr("record_iq"))
