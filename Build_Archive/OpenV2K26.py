...
# =============================================================================
#  HackRF detection -- targeted firmware version extraction
#  Avoids mistaking the hackrf_info / libhackrf tool version for FW version.
# =============================================================================
...
def detect_hackrf():
    try:
        ...
        if 'HackRF' in output and result.returncode == 0:
            ...
            #   e.g.  "Firmware Version: 2018.01.1 (API:1.06)"
            for l in lines:
            ...
            # Strategy 2: skip lines that are tool/library version announcements
            # (they always start with 'hackrf_info' or 'libhackrf').
            # Then grab the first 4-digit-year version number in any other line.
            # Covers formats like "Firmware Version: 2018.01.1" regardless of
            # whether (API:) is present or the casing/separator changes.
            if firmware is None:
                for l in lines:
                    ll = l.lower().lstrip()
            ...
            # Strategy 3: same prefix filter, but accept any version number on
            # a line that also contains the word 'firmware'
            if firmware is None:
                for l in lines:
                    ll = l.lower().lstrip()
                    if ll.startswith('hackrf_info') or ll.startswith('libhackrf'):
...
class SectionHeader(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Gradient: dark grey at top rule fading to window background colour
        ...
        # Top and bottom rules span the full widget width
        pen_rule = QtGui.QPen(QtGui.QColor("#888"), 1)
        p.setPen(pen_rule)
        p.drawLine(0, 0, w, 0)
        p.drawLine(0, self.height() - 1, w, self.height() - 1)

        # Centred text: 1px white drop shadow at (+1, +1) then main text
        text_x = (w - self._tw) // 2
...
class LabelledSlider(QtWidgets.QWidget):
    def __init__(self, label, lo, hi, step, default,
        ...
        # +20px wider than before; gap to readout removed (set to 0)
        ...
        # Gap removed: slider right edge now 20px closer to readout
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 26)")
        ...
        # Enable all optional filters by default
        # (called after blocks exist so callbacks can update filter taps)
    ...
    def _set_panel_active(self, panel, active):
        """Enable/disable panel with matching background colour."""
    ...
    def _keep_black(widget):
        """Force text to stay black even when the parent panel is disabled."""
        pal = widget.palette()
    ...
    @staticmethod
    def _style_green():
        # :disabled gives dark grey so buttons are obviously unavailable
    ...
    def _build_gui(self):
        ...
        # Always black and bold regardless of disabled state
        ...
        # Always black (not greyed when panel is disabled)
        ...
        # -- Right 20%: vertical duty cycle meter, centred horizontally ---
        dc_panel = QtWidgets.QWidget()
        dc_vbox  = QtWidgets.QVBoxLayout(dc_panel)
        # Small top margin; left/right margins let the bar centre naturally
        dc_vbox.setContentsMargins(4, 2, 4, 6); dc_vbox.setSpacing(0)

        # Percentage readout -- black bold, centre-aligned
        ...
        # Vertical bar centred in panel with fixed width
        ...
        self._dc_vbar.setFixedWidth(20)   # narrow bar, centred by stretchers
        ...
        # Centre the bar horizontally with equal stretchers
        ...
        # Description text -- same width as the Record IQ button below it
    ...
    def _cb_audio_swap(self):
        ...
        if self._audio_left_active:
            ...
            # Default mic to MUTED on every activation
    ...
    def _cb_output_swap(self):
        ...
        if self._output_left_active:
            # Restore hardware info detected at startup.
            # Re-running hackrf_info while osmosdr holds the device open
            # returns incomplete output and loses the firmware version.
            ...
            # Default TX to DISABLED on every activation
    ...
    def _cb_generate_espeak(self):
        ...
        if r.returncode != 0:
            self._espeak_status.setText(
                "eSpeak error: {}".format(r.stderr.strip()[:60])); return
        try:
