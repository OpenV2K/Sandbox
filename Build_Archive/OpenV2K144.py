...
class PulseHeaderWidget(QtWidgets.QWidget):
    ...
    digital signal trace: a single continuous stroked line, like a pen
    plotting a square wave, not filled rectangles.  During silence it
    rides flat along the baseline; at each pulse it draws straight up,
    across a flat top (one tick wide), straight back down, then
    continues along the baseline -- exactly what an oscilloscope shows
    for a digital pulse train, not a series of disconnected solid boxes.

    Still cheap to render: the whole trace for one full tick sequence is
    built ONCE at construction as a single QPainterPath of straight
    line segments (no curves, no per-frame rebuilding) -- only its
    on-screen TRANSLATION changes each frame, tiled and scrolled via the
    same modulo technique the wave used.

    Same background area, same width/position (wrapping the title
    label, child renders on top), and same rightward-only scroll
    direction as the wave version this replaced.

    Bitstream: a fixed-length tick sequence generated ONCE with Python's
    built-in random module -- lightweight, no extra dependency.  Each
    pulse tick is followed by a random 1-6 ticks of silence before the
    next pulse.  The generator keeps the final (pulse + silence run)
    unit whole rather than truncating mid-unit, so the sequence always
    ends in silence -- since it also always STARTS on a pulse, this
    guarantees the minimum-silence rule holds even at the seam where one
    loop of the animation rolls into the next.
    ...
    """
    TICK_PX = 8
    N_TICKS = 200          # pre-generated sequence length target (see
                          # _generate_bitstream for why actual length
                          # may run slightly past this)
    MIN_SILENCE = 1
    MAX_SILENCE = 6
    BASELINE_Y_PX = 44.0   # toward the bottom edge of the Event Log/Ref
                          ...
                          # vertically centred within this taller 46px
                          # row.  An estimate -- nudge directly if it
                          # doesn't land exactly once rendered.
    LINE_COLOR = QtGui.QColor(220, 220, 220)   # lightest grey from the
                                                # wave palette
    LINE_WIDTH_PX = 3
    SCROLL_PERIOD_S = 20.0   # was 10.0 -- 50% slower
    ...
    def _generate_bitstream(self):
        ...
        until at least N_TICKS long.  Generated once at construction
        (not per-frame -- that would be wasteful, since the pattern only
        needs to be decided once per launch) and reused for every scroll
        loop.  Unseeded, so it's genuinely different every time the app
        starts -- never the same pattern twice.
    ...
    def _build_trace_path(self):
        """One continuous oscilloscope-style trace for the full tick
        sequence: baseline, straight up at a pulse, flat across the top
        for one tick, straight back down, continue along the baseline.
        Starts and ends exactly at baseline level so consecutive tiled
        repeats connect seamlessly with no visible seam."""
        baseline_y = self.BASELINE_Y_PX
        top_y = 0.0
        path = QtGui.QPainterPath()
        path.moveTo(0.0, baseline_y)
        x = 0.0
        for is_pulse in self._ticks:
            if is_pulse:
                path.lineTo(x, top_y)
                path.lineTo(x + self.TICK_PX, top_y)
                path.lineTo(x + self.TICK_PX, baseline_y)
            else:
                path.lineTo(x + self.TICK_PX, baseline_y)
            x += self.TICK_PX
        return path
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Format: YYYY/M/D
        self.setWindowTitle("OpenV2K (2026/8/16 - Version 144)")
