...
# =============================================================================
#  Log Tab Button  (child widget; rounded right corners; "LOG >" / "< LOG")
# =============================================================================

class LogTabButton(QtWidgets.QAbstractButton):
    """
    A 26-px-wide vertical tab button painted with a blue gradient and rounded
    right corners.  Lives INSIDE the main window so it follows it for free.
    """
    W = 26; CORNER = 9
    ...
    def __init__(self, parent=None):
        ...
        self.setToolTip("Toggle event log")
    ...
    def paintEvent(self, event):
        ...
        w = float(self.W); h = float(self.height()); r = float(self.CORNER)

        # Tab shape: straight left edge (against app content),
        # rounded right corners (the visible "hanging" edge).
        ...
        # Blue fill -- brighter when expanded
        col = QtGui.QColor("#2a6ebb") if self.isChecked() \
              else QtGui.QColor("#1c4f8a")
        p.setPen(QtCore.Qt.NoPen)
        p.setBrush(col)
        p.drawPath(path)

        # Thin highlight on right/visible edges
        p.setPen(QtGui.QPen(QtGui.QColor(120, 180, 255, 160), 1))
        p.setBrush(QtCore.Qt.NoBrush)
        p.drawPath(path)

        # "< LOG" (open) or "LOG >" (closed), rotated top-to-bottom
        p.setPen(QtGui.QColor("white"))
        f = QtGui.QFont("Monospace", 7); f.setBold(True)
        p.setFont(f)
        p.translate(w, 0.0); p.rotate(90.0)
        label = "< LOG" if self.isChecked() else "LOG >"
        p.drawText(0, 0, int(h), int(w), QtCore.Qt.AlignCenter, label)


# =============================================================================
#  Log Drawer  (child widgets managed alongside app content in main HBoxLayout)
#
#  Architecture
#  ------------
#  The drawer and tab button are plain QWidget children of the main window's
#  central widget, laid out in the same QHBoxLayout as the app content.
#  Child widgets follow their parent without any tracking code -- this is the
#  only reliable approach on both X11 and Wayland.
#
#  When collapsed:  [App 580px][Tab 26px]                  total = 606px
#  When expanded:   [App 580px][Drawer 660px][Tab 26px]    total = 1266px
#
#  A QVariantAnimation drives the drawer's fixedWidth from 0 to 660 (open)
#  or 660 to 0 (close).  The window resizes automatically with its content.
# =============================================================================

class LogDrawer:
    """
    Manages the log tab button and the sliding drawer widget.
    Call add_to_layout(hbox) once to insert them into the main HBoxLayout.
    """
    LOG_W   = 660
    ANIM_MS = 280
    ...
    def __init__(self):
        ...
        self._drawer.setFixedWidth(0)   # starts collapsed
        dl = QtWidgets.QVBoxLayout(self._drawer)
        dl.setContentsMargins(0, 0, 0, 0); dl.setSpacing(0)

        hdr = QtWidgets.QLabel("  OpenV2K  Event Log")
        hdr.setStyleSheet(
        ...
        dl.addWidget(self._log_text)

        # Tab button
        self._tab = LogTabButton()
        self._tab.toggled.connect(self._on_toggle)

        # Width animation on the drawer widget
        ...
        self._anim.valueChanged.connect(
            lambda v: self._drawer.setFixedWidth(int(v)))
        self._anim.finished.connect(self._on_anim_done)

    def add_to_layout(self, hbox):
        """Insert drawer (hidden width) then tab into an HBoxLayout."""
        hbox.addWidget(self._drawer)
        hbox.addWidget(self._tab)

    # -- Toggle ---------------------------------------------------------------

    def _on_toggle(self, checked):
        """checked = True means opening the drawer."""
        self._expanded = checked
        self._anim.stop()
        start = self._drawer.width()
        end   = self.LOG_W if checked else 0
        self._anim.setStartValue(float(start))
        self._anim.setEndValue(float(end))
        self._tab.update()    # repaint label ("< LOG" / "LOG >")
        self._anim.start()

    def _on_anim_done(self):
        if not self._expanded:
            self._drawer.setFixedWidth(0)   # ensure fully collapsed

    # -- Logging --------------------------------------------------------------

    def _fmt(self, msg, in_progress=False):
        now = datetime.datetime.now()
        ms  = now.microsecond // 1000
        ts  = now.strftime("%H:%M:%S.") + "{:03d}".format(ms)
        return "[{}] {}{}".format(ts, msg, "   ..." if in_progress else ".")

    def log(self, msg, in_progress=False):
        """Append a timestamped entry. Thread-safe."""
        line = self._fmt(msg, in_progress)
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    APP_W              = 580   # fixed app content width
    AUDIO_RATE         = 48000
    HACKRF_RATE        = 2000000
    RESAMP_INTERP      = 125
    RESAMP_DECIM       = 3
    ...
    _TX_LICENSE = (
        ...
        "\n"
        "425 MHz -- 70cm UHF band.\n"
        "Technician class or higher (US/FCC).\n"
        "\n"
        "1300 MHz -- 23cm microwave band.\n"
        "Technician class or higher (US/FCC).\n"
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 31)")
        # Minimum width = app content + tab; expands when drawer opens
        self.setMinimumWidth(self.APP_W + LogTabButton.W)
        ...
        # Log drawer -- created before _build_gui so _log() can be called
        self._log = LogDrawer()
    ...
    def _build_gui(self):
        ...
        # Outer layout: [app content 580px] [drawer 0-660px] [tab 26px]
        # Drawer and tab are child widgets -- they follow the window for free.
        h_outer = QtWidgets.QHBoxLayout(central)
        h_outer.setContentsMargins(0, 0, 0, 0); h_outer.setSpacing(0)

        # App content area (fixed 580px)
        content_w = QtWidgets.QWidget()
        content_w.setFixedWidth(self.APP_W)
        vbox = QtWidgets.QVBoxLayout(content_w)
        vbox.setContentsMargins(12, 12, 12, 12); vbox.setSpacing(0)
        h_outer.addWidget(content_w)

        # Insert log drawer + tab into the outer layout
        self._log.add_to_layout(h_outer)
        ...
        title = QtWidgets.QLabel(
            ...
            "</h3><small>Zero-Crossing Pulse Transmitter</small>")
        ...
        meter_lbl = QtWidgets.QLabel(
            "Mic Level: silence -45 dB,\naim for -18dB when speaking.\n"
        ...
        self._sl_pulse = LabelledSlider("Pulse (us)", 50, 500, 5,
            self._pulse_us, fmt="{:.0f} us", callback=self._cb_pulse, tick_steps=10)
        left_vbox.addWidget(self._sl_pulse)
        self._sl_hpf = LabelledSlider("HPF (Hz)", 250, 1000, 10,
            self._hpf_hz, fmt="{:.0f} Hz", callback=self._cb_hpf, tick_steps=10)
        left_vbox.addWidget(self._sl_hpf)
        self._sl_lpf = LabelledSlider("LPF (Hz)", 1000, 15000, 100,
            self._lpf_hz, fmt="{:.0f} Hz", callback=self._cb_lpf, tick_steps=5)
        ...
        lic_lbl = QtWidgets.QLabel(self._TX_LICENSE)
        lic_lbl.setStyleSheet("color:#666; font-size:9px;")
        ...
        self._save_path_lbl.setStyleSheet("color:#777; font-size:11px;")
    ...
    def _cb_audio_swap(self):
        ...
        if self._audio_left_active:
            ...
            self._log.log("Input: Live Microphone (muted)")
        else:
            self._set_panel_active(self._mic_panel,False)
            self._set_panel_active(self._es_panel,True)
            ...
            self._log.log("Input: eSpeak TTS")
    ...
    def _cb_output_swap(self):
        ...
        if self._output_left_active:
            ...
            self._btn_tx.setStyleSheet(self._style_red())
            self._log.log("Output: HackRF Transmitter (disabled)")
        else:
            self.tx_gate.set_k((0+0j))
            if not self._btn_tx.isChecked(): self._btn_tx.setChecked(True)
            self._set_panel_active(self._tx_panel,False)
            self._set_panel_active(self._save_panel,True)
            self._log.log("Output: Save to Disk")
    ...
    def _cb_mute(self,muted):
        ...
        self._log.log("Microphone muted" if muted else "Microphone live -- monitoring")
    ...
    def _cb_generate_espeak(self):
        ...
        self._log.log("Generating eSpeak audio",in_progress=True)
        ...
        if cmd_found is None:
            ...
            self._log.log("ERROR: espeak-ng not installed"); return

        r=subprocess.run([cmd_found,'-w',self.ESPEAK_RAW,text],
                         capture_output=True,text=True,timeout=15)
        if r.returncode!=0:
            err=r.stderr.strip()[:80]
            self._espeak_status.setText("eSpeak error: {}".format(err))
            self._log.log("ERROR eSpeak: {}".format(err)); return
        ...
        try:
            ...
            self._espeak_status.setText("WAV error: {}".format(e))
            self._log.log("ERROR WAV: {}".format(e)); return
        ...
        silence=np.zeros(int(self.AUDIO_RATE*self.ESPEAK_SILENCE_SEC),
                         dtype=np.float32)
        ...
        try:
            ...
            self._log.log("ERROR GR reload: {}".format(e)); return

        self.espeak_gate.set_k(1.0); self.mic_gate.set_k(0.0)
        self.mute_gate.set_k(1.0)
        short=text[:30]+("..." if len(text)>30 else "")
        self._espeak_status.setText('Playing: "{}"'.format(short))
        self._log.log('eSpeak playing: "{}"'.format(short))
    ...
    def _cb_record_toggle(self,recording):
        if recording:
            ...
            try:
                ...
                self._log.log("ERROR starting record: {}".format(e)); return
            ...
            self._log.log("Recording IQ to disk",in_progress=True)
        else:
            try:
                self.lock()
                self.disconnect(self.f2c,self.iq_recorder)
    ...
    def _generate_waterfall(self,iq_path):
        try:
            ...
            if len(data)<1024:
                self._set_wf_status("Recording too short")
                self._log.log("Waterfall: recording too short"); return
            ...
            self._set_wf_status("Saved: {}".format(os.path.basename(png)))
            self._log.log("Waterfall saved: {}".format(os.path.basename(png)))
        except Exception as e:
            self._set_wf_status("Error: {}".format(e))
            self._log.log("ERROR waterfall: {}".format(e))
