...
class SimpleNoiseGate(gr.sync_block):
    ...
    def set_enabled(self, enabled): self._enabled = bool(enabled)
...
# =============================================================================
#  Section Header widget  (tab-style, draws own top rule)
# =============================================================================

class SectionHeader(QtWidgets.QWidget):
    """
    Bold monospace section header with tab-style border.

    Draws:
      y=0  ===========================  full-width rule
                        |               vertical line
           HEADER TEXT  |
           =============+               short underline + corner
    """
    _PT  = 12
    _GAP = 7
    _ML  = 2
...
# =============================================================================
#  Swap Button widget  (circular toggle with vertical connector lines)
# =============================================================================

class SwapButton(QtWidgets.QWidget):
    """
    Circular '<>' button centred in a vertical connector.
    Vertical lines run from the widget's top and bottom edges to the
    button's circumference, making it clear only one side is active at a
    time.  Emits clicked() on press.
    """
    BTN_D = 30     # button diameter (px)
    W     = 44     # total widget width (px)
    ...
    def paintEvent(self, event):
        ...
        btn_top = mid - half_d
        btn_bot = mid + half_d
        p.drawLine(cx, 0,       cx, btn_top)
        p.drawLine(cx, btn_bot, cx, self.height())
...
class LabelledSlider(QtWidgets.QWidget):
    def __init__(self, label, lo, hi, step, default,
        ...
        self._slider.setMinimumWidth(200); self._slider.setMaximumWidth(320)
        ...
        lbl.setMinimumWidth(88); lbl.setMaximumWidth(96)
        row = QtWidgets.QHBoxLayout(self)
        row.setContentsMargins(0, 0, 0, 0); row.setSpacing(4)
        row.addWidget(lbl); row.addWidget(self._slider)
        row.addSpacing(36); row.addWidget(self._readout)
...
class OpenV2K(gr.top_block, QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("OpenV2K (2026/7/23 - Version 15)")
        ...
        self._muted             = True    # mic mute state
        self._tx_enabled        = False
        self._recording         = False
        self._record_path       = None
        self._audio_left_active = False   # False = eSpeak (right) active
        self._output_left_active= False   # False = Save to Disk (right) active
    ...
    def _build_gui(self):
        ...
        vbox.addSpacing(2)     # tight gap so swap button lines nearly reach rule
        ...
        meter_lbl = QtWidgets.QLabel(
            ...
            "Adjust gain in System Settings\n"
            "> Sound > Input Volume\n"
        ...
        # -- Swap button (audio) ----------------------------------------------
        self._audio_swap_btn = SwapButton()
        self._audio_swap_btn.clicked.connect(self._cb_audio_swap)
        audio_row.addWidget(self._audio_swap_btn)
        ...
        vbox.addSpacing(2)     # tight gap so swap button lines nearly reach next rule
        ...
        self._dc_bar.setRange(0, 1000); self._dc_bar.setValue(0)
        self._dc_bar.setTextVisible(False); self._dc_bar.setFixedHeight(14)
        ...
        # =====================================================================
        # Section 3: SDR Output  (hline only, no header label)
        # =====================================================================
        vbox.addWidget(self._hline())
        vbox.addSpacing(2)     # tight gap to match swap button lines
        ...
        # -- Swap button (output) ---------------------------------------------
        self._output_swap_btn = SwapButton()
        self._output_swap_btn.clicked.connect(self._cb_output_swap)
        output_row.addWidget(self._output_swap_btn)
        ...
        self._save_path_lbl = QtWidgets.QLabel(
            "Records raw complex64 IQ.\n"
            "Waterfall via numpy FFT\n"
            "and matplotlib (inferno).")
        ...
        #   Input  -> eSpeak active (right), mic disabled (left)
        #   Output -> Save to Disk active (right), HackRF disabled (left)
        # =====================================================================
    ...
    def _build_blocks(self):
        ...
        # Default: eSpeak active, mic silent
        ...
        self.mute_gate   = blocks.multiply_const_ff(0.0)   # starts muted
    ...
    def _toggle_notch(self, enabled):
        self.notch.set_taps(self._notch_b if enabled else [1.0],
                            self._notch_a if enabled else [0.0])
    ...
    def _cb_audio_swap(self):
        """Toggle between Live Microphone (left) and eSpeak TTS (right)."""
    ...
    def _cb_output_swap(self):
        ...
        if self._output_left_active:
            ...
            # Save to Disk is now active
            # Safety: disable TX when leaving HackRF mode
    ...
    def _cb_mute(self, muted):
        """Controls the overall mute gate. Source selection is handled by the swap button."""
        self._muted = muted
        self.mute_gate.set_k(0.0 if muted else 1.0)
        if muted:
    ...
    def _cb_generate_espeak(self):
        ...
        # Ensure eSpeak gate is open and unmute the chain
        self.espeak_gate.set_k(1.0)
        self.mic_gate.set_k(0.0)
    ...
    def _generate_waterfall(self, iq_path):
        try:
            ...
            if len(data) < 1024:
                self._set_wf_status("Recording too short for waterfall")
                return
            ...
            self._set_wf_status("Waterfall error: {}".format(e))
